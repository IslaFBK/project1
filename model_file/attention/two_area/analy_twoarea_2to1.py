...
# -*- coding: utf-8 -*-
"""
Created on Tue Jan 12 17:46:27 2021
...
for b in range(len(fr_bin)):
    for i in range(N_stim):
        ...
        #data.a1.ge.spk_matrix[]
            ...
            hz_loc[i, j, b] = data.a1.ge.spk_matrix[n_in_bin[j], data.a1.param.stim.stim_on[i,0]*10:(data.a1.param.stim.stim_on[i,0]+fr_bin[b])*10].sum()/n_in_bin[j].shape[0]/(fr_bin[b]/1000)
#%%

hz_loc_spon = np.zeros([2, len(n_in_bin)])
'''no attention'''
for j in range(len(n_in_bin)):
    hz_loc_spon[0, j] = data.a1.ge.spk_matrix[n_in_bin[j], 5000*10:10000*10].sum()/n_in_bin[j].shape[0]/5
'''attention'''
spon_adpt_stt = data.a1.param.stim.stim_on[30,0] - 2000 
spon_adpt_end = data.a1.param.stim.stim_on[30,0]
for j in range(len(n_in_bin)):
    hz_loc_spon[1, j] = data.a1.ge.spk_matrix[n_in_bin[j], spon_adpt_stt*10:spon_adpt_end*10].sum()/n_in_bin[j].shape[0]/2
...
if loop_num%5 == 0:
    ...
    hz_t_mean_noatt_1 = hz_t[:N_stim_noatt,:,0].reshape(3,10,-1).mean(1)
    hz_t_mean_noatt_2 = hz_t[:N_stim_noatt,:,1].reshape(3,10,-1).mean(1)
    hz_t_mean_att_1 = hz_t[N_stim_noatt:,:,0].reshape(3,10,-1).mean(1)
    hz_t_mean_att_2 = hz_t[N_stim_noatt:,:,1].reshape(3,10,-1).mean(1)
    
    fig, ax = plt.subplots(1,2,figsize=[10,5])
    for i in range(3):
        ax[0].plot(hz_t_mean_noatt_1[i], c=clr[i], ls='--',label = 'stim_amp: %.1f Hz; a1'%(stim_amp[i]))
        ax[1].plot(hz_t_mean_noatt_2[i], c=clr[i], ls='--', label = 'stim_amp: %.1f Hz; a2'%(stim_amp[i]))
    for i in range(3):
        ax[0].plot(hz_t_mean_att_1[i], c=clr[i], ls='-',label = 'attention; stim_amp: %.1f Hz; a1'%(stim_amp[i]))
        ax[1].plot(hz_t_mean_att_2[i], c=clr[i], ls='-', label = 'attention; stim_amp: %.1f Hz; a2'%(stim_amp[i]))
    for axi in range(2):    
        ax[axi].set_ylim([min(ax[0].get_ylim()[0], ax[1].get_ylim()[0]), max(ax[0].get_ylim()[1], ax[1].get_ylim()[1])])                                 
...
moviefile = savetitle+'_spon'+'_%d'%loop_num+'.mp4'
if loop_num%5 == 0:
    ani.save(moviefile)
    pass
#%%
