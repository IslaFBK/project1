...
# -*- coding: utf-8 -*-
"""
Created on Mon Apr 12 11:57:33 2021
...
for ie_r_i1 in [1.52]:#np.arange(1.40,1.521,0.02):#[1.40]:
    for w_extnl_ in [10]:
        for delta_gk_1 in [3.5]:#np.linspace(7,15,9)]]:
            for delta_gk_2 in [16]:
                for new_delta_gk_2 in [0.5]:#np.arange(0.15,0.31,0.05)*delta_gk_:#np.linspace(1,5,5):
                    for tau_s_di_ in np.linspace(4.4,4.4,1):
                        for tau_s_de_ in np.linspace(5.,5.,1):
                            for tau_s_r_ in [1]:#np.linspace(1,1,1):
                                for scale_w_12_e in [2.6]:#np.arange(2, 2.81, 0.2):#np.arange(0.8,1.21,0.05):[2.5]
...
ie_r_e = 2.76*6.5/5.8; ie_r_e2 = 1.06
ie_r_i = 2.450*6.5/5.8; ie_r_i2 = 1.06
...
syn_e1e2, syn_e1i2, syn_e2e1, syn_e2i1 = twoarea_net.build(ijwd1, ijwd2, ijwd_inter, adapt_modu_1 = False, adapt_modu_2 = False)
#%%
...
for i in range(n_StimAmp):
    stim_amp_scale[i*n_perStimAmp:i*n_perStimAmp+n_perStimAmp] = 2#2**i

stim_scale_cls.stim_amp_scale = stim_amp_scale
stim_scale_cls.stim_dura = 10000
stim_scale_cls.separate_dura = np.array([500,800])
...
# dgk_modu = stim_scale_cls.scale_stim.copy()

# for dura in stim_scale_cls.stim_on:
#     dgk_modu[int(round(dura[0]/stim_scale_cls.dt_stim)): int(round(dura[0]/stim_scale_cls.dt_stim))+int(round((dura[1]-dura[0])/2/stim_scale_cls.dt_stim))] = 0

# dgk_modu[dgk_modu!=0] = 1
# dgk_modu_on = TimedArray(dgk_modu, dt=coef10*ms)
#%%
# fid,ax=plt.subplots(1,1)
# ax.plot(stim_scale_cls.scale_stim[19000:20000])
# ax.plot(dgk_modu[19000:20000])
...
stim_scale_cls_att.stim_dura = 10000
stim_scale_cls_att.separate_dura = np.array([500,800])
...
# group_e_2.delta_gk_base = delta_gk_2*nS
# group_e_2.delta_gk_modu = (adapt_value-delta_gk_2)*nS
