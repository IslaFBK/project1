...
# -*- coding: utf-8 -*-
"""
Created on Sat Mar 13 11:41:48 2021
...
if get_HzTemp:
    ...
    for mua_loca_1 in [[0,0]]:#,[-32,-32]]:
        ...
        dura_onoff[:,0] -= 100
        #dura_onoff[:,1] += 100
        dura_onoff[:,1] = dura_onoff[:,0] + 5100
...
if get_HzTemp2:
    ...
    for mua_loca_1 in [[0,0]]:#,[-32,-32]]:
        ...
        dura_onoff[:,0] -= 100
        #dura_onoff[:,1] += 100
        dura_onoff[:,1] = dura_onoff[:,0] + 2100
        hz_t = fra.firing_rate_time_multi(data.a1.ge, mua_neuron_1, dura_onoff, window=window)
        data_anly.hz_t = hz_t.copy()
        ...
        for i in range(n_StimAmp):
            ...
            #for i in range(n_amp_stim_att):
            t_plot = np.arange(dura_onoff[0,1] - dura_onoff[0,0]) - 100
            ax.plot(t_plot, hz_t_noatt_mean, ls='--', c=clr[i], label = 'stim_amp: %.1f Hz;\nloc: [%.1f,%.1f]'%(stim_amp[i],mua_loca_1[0],mua_loca_1[1]))
            ax.fill_between(t_plot, hz_t_noatt_mean-hz_t_noatt_sem, hz_t_noatt_mean+hz_t_noatt_sem, \
                             ls='--', facecolor=clr[i], edgecolor=clr[i], alpha=0.2)
            #for i in range(n_amp_stim_att,n_amp_stim):
            ax.plot(t_plot, hz_t_att_mean, ls='-', c=clr[i], label = 'att; stim_amp: %.1f Hz;\nloc: [%.1f,%.1f]'%(stim_amp[i],mua_loca_1[0],mua_loca_1[1])) 
            ax.fill_between(t_plot, hz_t_att_mean-hz_t_att_sem, hz_t_att_mean+hz_t_att_sem, \
                             ls='-', facecolor=clr[i], edgecolor=clr[i], alpha=0.2)
    
    ax.set_xlim([t_plot[0]+50, t_plot[-1]+100])
    ax.set_xlabel('ms')
    ax.set_ylabel('Hz')
    #title = "2e1e:%.3f_2e1i:%.3f_1hz:%.2f_2hz:%.2f"%(scale_w_21_e[ie_ind//scale_w_21_i.shape[0]], scale_w_21_i[ie_ind%scale_w_21_i.shape[0]], spon_rate1,spon_rate2)
    #title = ''
    ax.set_title(title + '; senssory')
    ax.legend()
    if save_img: fig.savefig(title.replace('\n','')+'_2temp'+'_%d'%loop_num+'.png')
...
if get_ani:
    ...
    start_time = data.a1.param.stim1.stim_on[first_stim,0] - 3000
    ...
    ani = fra.show_pattern(spkrate1=data.a1.ge.spk_rate.spk_rate, spkrate2=data.a2.ge.spk_rate.spk_rate, \
                                            ax1title='sens', ax2title='asso', \
                                            frames = frames, start_time = start_time, interval_movie=15, anititle='spontaneous',stim=None, adpt=None)
    savetitle = title.replace('\n','')
    ...
    end_time = data.a1.param.stim1.stim_on[last_stim,0] + 2000
    ...
    ani = fra.show_pattern(spkrate1=data.a1.ge.spk_rate.spk_rate, spkrate2=data.a2.ge.spk_rate.spk_rate, \
                           ax1title='sens', ax2title='asso', \
                           frames = frames, start_time = start_time, interval_movie=15, anititle='stim in sens; no attention',stim=stim, adpt=adpt)
    savetitle = title.replace('\n','')
    ...
    ani = fra.show_pattern(spkrate1=data.a1.ge.spk_rate.spk_rate, spkrate2=data.a2.ge.spk_rate.spk_rate, \
                           ax1title='sens', ax2title='asso',                
                           frames = frames, start_time = start_time, interval_movie=15, anititle='stim in sens; attention on',stim=stim, adpt=adpt)
    savetitle = title.replace('\n','')
