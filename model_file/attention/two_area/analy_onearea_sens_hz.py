...
# -*- coding: utf-8 -*-
"""
Created on Mon Jan 11 17:03:50 2021
...
#%%
fr_bin = np.array([200])#np.array([150, 200, 250]) 
...
for j in range(len(n_in_bin)):
    hz_spon[j] = data.a1.ge.spk_matrix[n_in_bin[j], 5000*10:10000*10].sum()/n_in_bin[j].shape[0]/5
...
if loop_num%5 == 0:
    ...
    hz_t_mean = hz_t.reshape(5,10,-1).mean(1)
    ...
    plt.xlabel('ms')
    plt.ylabel('Hz')
    plt.title('average firing rate versus time (10 trials)\n'+title)
    savetitle = title.replace('\n','')
