...
# -*- coding: utf-8 -*-
"""
Created on Tue Mar 16 15:51:23 2021
                ...
                for tau_s_di_ in np.linspace(4.4,4.4,1):
                    for tau_s_de_ in np.linspace(5.,5.,1):
                        for tau_s_r_ in [1]:#np.linspace(1,1,1):
                            for scale_w_12_e in [2.5]:#np.arange(1.6, 2.01, 0.05):#np.arange(0.8,1.21,0.05):
                                for scale_w_12_i in scale_w_12_e*np.array([1]):#np.array([0.8,0.9,1.0,1.1]):#np.arange(0.9,1.11,0.1):#np.arange(0.8,1.21,0.05):#[scale_w_12_e]:#0.2*np.arange(0.8,1.21,0.05):
                                    for scale_w_21_e in [0.3]:#np.arange(0.3, 0.71, 0.05):#np.arange(0.8,1.21,0.05):#[1]:#np.arange(0.8,1.21,0.05):
                                        for scale_w_21_i in [scale_w_21_e*0.95]:#np.array([0.95,1.0,1.05]):#np.arange(0.9,1.11,0.1):#np.arange(0.8,1.21,0.05):#[1]:#[scale_w_21_e]:#0.35*np.arange(0.8,1.21,0.05):
                                            for tau_p_d_e1_e2 in [5]:
                                                for tau_p_d_e1_i2 in [5]:
                                                    for tau_p_d_e2_e1 in [10]:#np.arange(7,14.1,1):
                                                        for tau_p_d_e2_i1 in [10]:#np.arange(7,14.1,1):#[10]:
...
#     stim_amp_scale[i*n_perStimAmp:i*n_perStimAmp+n_perStimAmp] = 3
...
#seed(10)
simu_time_tot = 7000*ms#(stim_scale_cls.stim_on[-1,1] + 500)*ms
#simu_time1 = 10000*ms#2000*ms
simu_time1 = simu_time_tot#(stim_scale_cls.stim_on[0,1] + 2000)*ms
...
#extnl_e1.rates = 0*Hz
trial = 100
spk_data_ge1 = [None]*trial
spk_data_gi1 = [None]*trial
spk_data_ge2 = [None]*trial
spk_data_gi2 = [None]*trial
...
#net.run(simu_time2, report = 'text', profile=False) #,namespace={'tau_k': 80*ms}
    ...
    spk_data_ge1[tr] = {'i':spk_e_1.i[:].copy(),'t':spk_tstep_e1}
    spk_data_gi1[tr] = {'i':spk_i_1.i[:].copy(),'t':spk_tstep_i1}
    spk_data_ge2[tr] = {'i':spk_e_2.i[:].copy(),'t':spk_tstep_e2}
    spk_data_gi2[tr] = {'i':spk_i_2.i[:].copy(),'t':spk_tstep_i2}
    #print(spk_e_1.i[:2])
    print(spk_data_ge1[tr]['i'][:2])
    net.restore('state1')
#%%
# spk_tstep_e1 = np.round(spk_e_1.t/(0.1*ms)).astype(int)
# spk_tstep_i1 = np.round(spk_i_1.t/(0.1*ms)).astype(int)
