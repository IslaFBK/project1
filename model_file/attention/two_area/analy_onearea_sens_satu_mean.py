...
# -*- coding: utf-8 -*-
"""
Created on Sun Mar 14 12:12:30 2021
...
'''tunning curve'''
if get_TunningCurve:
    hz_loc_mean = np.zeros([n_StimAmp, data_anly.hz_loc.shape[1], repeat]);
    hz_loc_spon_mean = np.zeros([1, data_anly.hz_loc_spon.shape[1], repeat]); 
...
if get_HzTemp:
    hz_t_mean = np.zeros([n_StimAmp, data_anly.hz_t.shape[1], repeat]); 
    hz_loc_elec_mean = np.zeros([n_StimAmp*1+1, data_anly.hz_loc_elec_mean.shape[1], repeat])
    hz_t_mean[:] = np.nan
...
for i in range(0, 121):
    ...
    for loop_num in range(i*repeat, (i+1)*repeat):
        try: 
            ...
            continue
                #divd -= 1
        if get_TunningCurve:                    
            hz_loc_mean[:,:,loop_num%repeat] = np.nanmean(data_anly.hz_loc.reshape(n_StimAmp, n_perStimAmp, -1), 1)
            hz_loc_spon_mean[:,:,loop_num%repeat] = data_anly.hz_loc_spon
        if get_HzTemp:
            hz_t_mean[:,:,loop_num%repeat] = np.nanmean(data_anly.hz_t.reshape(n_StimAmp, n_perStimAmp, -1), 1)
    ...
    title += '_1hz%.2f'%(spon_rate_mean[0])
    ...
    if get_HzTemp:
        ...
        dura_onoff = data.a1.param.stim.stim_on[0,1] + 100 - data.a1.param.stim.stim_on[0,0] + 100
