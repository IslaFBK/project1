...
# -*- coding: utf-8 -*-
"""
Created on Sun Apr 25 23:44:30 2021
...
savefile_name = 'data_anly_tc' #'data_anly' data_anly_temp data_anly_nc_normal
save_apd = '_tc'
...
if fftplot and save_img:
    ...
    fig.suptitle(title1)
    fftfile = savetitle+'_fft'+save_apd+'_%d'%(loop_num)+'.png'
    fig.savefig(fftfile)
    plt.close()
...
if getfano:
    ...
    win_lst = [50, 100, 230] # [50, 100, 230, 500]
...
if get_nscorr:
    ...
    for st in range(n_StimAmp): 
        '''no-att; within 1 group2'''
        nscorr.dura1 = data.a1.param.stim1.stim_on[st*n_perStimAmp:(st+1)*n_perStimAmp]
        nscorr.dura2 = None
        corr_mean, corr_sem = nscorr.get_nc_withingroup()
        ns2[st, 0, 0], ns2[st, 0, 1] = corr_mean, corr_sem   
        '''att; within 1 group2'''
        nscorr.dura1 = data.a1.param.stim1.stim_on[(st+n_StimAmp)*n_perStimAmp:(st+n_StimAmp+1)*n_perStimAmp]
        nscorr.dura2 = None
        corr_mean, corr_sem = nscorr.get_nc_withingroup()
        ns2[st, 1, 0], ns2[st, 1, 1] = corr_mean, corr_sem
    
    data_anly.nscorr.nscorr2 = ns2
    
    fig, ax = plt.subplots(2,1, figsize=[6,8])
    ax[0].errorbar(np.arange(len(stim_amp)), ns[0,:,0,0], ns[0,:,0,1], c=clr[0], fmt='--', marker='o', label='no-att;group 1')
    ax[0].errorbar(np.arange(len(stim_amp)), ns[0,:,1,0], ns[0,:,1,1], c=clr[1], fmt='-', marker='o', label='att;group 1')
    ax[0].errorbar(np.arange(len(stim_amp)), ns[1,:,0,0], ns[1,:,0,1], c=clr[0], fmt='--', marker='x', label='no-att;between-groups')
    ax[0].errorbar(np.arange(len(stim_amp)), ns[1,:,1,0], ns[1,:,1,1], c=clr[1], fmt='-', marker='x', label='att;between-groups')
    
    ax[1].errorbar(np.arange(len(stim_amp)), ns2[:,0,0], ns2[:,0,1], c=clr[0], fmt='--', marker='o', label='no-att;group 2')
    ax[1].errorbar(np.arange(len(stim_amp)), ns2[:,1,0], ns2[:,1,1], c=clr[1], fmt='-', marker='o', label='att;group 2')
    ...
    nsfile = savetitle+'_nc'+save_apd+'_%d'%(loop_num)+'.png'
...
if get_nscorr_t:
    ...
    neu_pool[1] = neuron[(dist >= 0) & (dist <= neu_range)]
        ...
        # neu_range = 5
        # neu_pool[0] = neuron[(dist >= 0) & (dist <= neu_range)]
    win_lst = [50, 100, 150] #[50, 100, 150, 300, 500]
    ...
    for win in win_lst:
        ...
        for st in range(n_StimAmp):  
            ax[0].errorbar(sample_t, ns_t[0, 0, :, st],ns_t[0, 1, :, st], c=clr[st], fmt='--', marker='o', label='no-att;group 1;amp:%.1fHz'%stim_amp[st])
            ax[0].errorbar(sample_t, ns_t[0, 0, :, st+n_StimAmp],ns_t[0, 1, :, st+n_StimAmp], c=clr[st], fmt='-', marker='o', label='att;group 1;amp:%.1fHz'%stim_amp[st])
            ax[1].errorbar(sample_t, ns_t[1, 0, :, st],ns_t[1, 1, :, st], c=clr[st], fmt='--', marker='x', label='no-att;between groups;amp:%.1fHz'%stim_amp[st])
            ax[1].errorbar(sample_t, ns_t[1, 0, :, st+n_StimAmp],ns_t[1, 1, :, st+n_StimAmp], c=clr[st], fmt='-', marker='x', label='att;between groups;amp:%.1fHz'%stim_amp[st])
            ax[2].errorbar(sample_t, ns_t[2, 0, :, st],ns_t[2, 1, :, st], c=clr[st], fmt='--', marker='o', label='no-att;group 2;amp:%.1fHz'%stim_amp[st])
            ax[2].errorbar(sample_t, ns_t[2, 0, :, st+n_StimAmp],ns_t[2, 1, :, st+n_StimAmp], c=clr[st], fmt='-', marker='o', label='att;group 2;amp:%.1fHz'%stim_amp[st])
        ...
        savetitle = title.replace('\n','')
        nsfile = savetitle+'_nct_win%.1f'%win+save_apd+'_%d'%(loop_num)+'.png'
        if save_img: fig.savefig(nsfile)
        plt.close()
#%%
...
if get_TunningCurve:
    ...
    elec_posi[:,1] = elec_posi_x # for diagonal electrode
    
    mua_range = 2
    #%%
    for elec in elec_posi:
        n_in_bin.append(cn.findnearbyneuron.findnearbyneuron(data.a1.param.e_lattice, elec, mua_range, data.a1.param.width))
    ...
    '''no attention'''
    spon_onoff = np.array([[5000,20000]])
    hz_loc_spon[0,:] = fra.tuning_curve(data.a1.ge.spk_matrix, spon_onoff, n_in_bin)
    
    '''attention'''
    spon_adpt_stt = data.a1.param.stim1.stim_on[N_stim,0] - 2000 
    spon_adpt_end = data.a1.param.stim1.stim_on[N_stim,0]
    spon_onoff = np.array([[spon_adpt_stt,spon_adpt_end]])
    hz_loc_spon[1,:] = fra.tuning_curve(data.a1.ge.spk_matrix, spon_onoff, n_in_bin)
...
if get_ani:
    ...
    moviefile = savetitle+'_spon'+save_apd+'_%d'%loop_num+'.mp4'
    ...
    end_time = data.a1.param.stim1.stim_on[last_stim,0] + 1500
    ...
    moviefile = savetitle+'_noatt'+save_apd+'_%d'%loop_num+'.mp4'
    ...
    '''no att; high input'''
    """
    first_stim = 2*n_perStimAmp -1; last_stim = 2*n_perStimAmp
    start_time = data.a1.param.stim1.stim_on[first_stim,0] - 300
    ...
    moviefile = savetitle+'_noatt_hipt'+save_apd+'_%d'%loop_num+'.mp4'
    ...
    #first_stim = 1*n_perStimAmp -1 + n_perStimAmp*n_StimAmp; last_stim = 1*n_perStimAmp + n_perStimAmp*n_StimAmp
    first_stim = n_perStimAmp; last_stim = n_perStimAmp
    start_time = data.a1.param.stim1.stim_on[first_stim,0] - 300
    end_time = data.a1.param.stim1.stim_on[last_stim,0] + 1500
    ...
    moviefile = savetitle+'_att'+save_apd+'_%d'%loop_num+'.mp4'
    ...
    moviefile = savetitle+'_att_hipt'+save_apd+'_%d'%loop_num+'.mp4'
