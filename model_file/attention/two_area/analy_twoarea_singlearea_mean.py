...
# -*- coding: utf-8 -*-
"""
Created on Fri Feb 12 16:47:46 2021
...
repeat = 30
para_n = 1#81
mua_loca_all = np.array([[0,0],[-32,-32],[0,32]])
divd = repeat
...
coef_spon_a1_mean = np.zeros([repeat, data_anly.coef_spon_a1.shape[0]])
coef_spon_a1_mean[:] = np.nan
freq_spon_a2 = data_anly.freq_spon_a2
coef_spon_a2_mean = np.zeros([repeat, data_anly.coef_spon_a2.shape[0]])
coef_spon_a2_mean[:] = np.nan
                
freq_adapt_a2 = data_anly.freq_adapt_a2
coef_adapt_a2_mean = np.zeros([repeat, data_anly.coef_adapt_a2.shape[0]])
coef_adapt_a2_mean[:] = np.nan


hz_loc = np.zeros([repeat, 3, data_anly.hz_loc.shape[1]])# = hz_loc
hz_loc_spon = np.zeros([repeat, data_anly.hz_loc_spon.shape[1]])#data_anly.hz_loc_spon = hz_loc_spon
hz_t = np.zeros([repeat, 3, data_anly.hz_t.shape[1]])
...
for para in range(para_n):
    ...
    for mua_loc_ind in range(1):#3
        ...
        for loop_num in range(para*repeat, para*repeat+repeat):
            ...
            coef_spon_a1_mean[loop_num - para*repeat] = np.abs(data_anly.coef_spon_a1)
            coef_spon_a2_mean[loop_num - para*repeat] = np.abs(data_anly.coef_spon_a2)
            coef_adapt_a2_mean[loop_num - para*repeat] = np.abs(data_anly.coef_adapt_a2)
            hz_loc[loop_num - para*repeat] = data_anly.hz_loc.reshape(3,30,-1).mean(1)
            hz_loc_spon[loop_num - para*repeat] = data_anly.hz_loc_spon[0]
            hz_t[loop_num - para*repeat] = data_anly.hz_t.reshape(3,30,-1).mean(1)
            ...
            spon_rate_1 += data_anly.spon_rate1
            spon_rate_2 += data_anly.spon_rate2
        ...
        #MI_surr /= divd
        #MSD /= divd
        coef_spon_a1_mean_reliz = np.nanmean(coef_spon_a1_mean,0)
        coef_spon_a2_mean_reliz = np.nanmean(coef_spon_a2_mean,0)
        coef_adapt_a2_mean_reliz = np.nanmean(coef_adapt_a2_mean,0)
        hz_loc_mean_reliz = np.nanmean(hz_loc,0)
        hz_loc_sem_reliz = scipy.stats.sem(hz_loc, 0, nan_policy='omit')
        hz_loc_spon_mean_reliz = np.nanmean(hz_loc_spon,0)
        hz_loc_spon_sem_reliz = scipy.stats.sem(hz_loc_spon, 0, nan_policy='omit')
        hz_t_mean_reliz = np.nanmean(hz_t,0)
        hz_t_sem_reliz = scipy.stats.sem(hz_t, 0, nan_policy='omit')
        
        
        spon_rate_1 /= divd; #print(spon_rate)
        spon_rate_2 /= divd;
        
        title = 'two'
        '''sens; fft'''
        pkf_spon = find_peakF(coef_spon_a1_mean_reliz, freq_spon_a1, 3)#freq_spon[1:][coef_spon_mean[1:].argmax()]
        #pkf_adapt = find_peakF(coef_adapt_a2_mean_reliz, freq_adapt_a2, 3)#freq_adapt[1:][coef_adapt_mean[1:].argmax()]
        
        title_a1 = title + '_a1_hz%.2f_pkfspon%.2f'%(spon_rate_1,pkf_spon)
        '''fft'''
        fig, ax = plt.subplots(2,2,figsize=[9,9])
        fig, ax = plot_fft(freq_spon_a1, coef_spon_a1_mean_reliz, freq_max1=20, freq_max2 = 200, fig=fig, ax=ax, label='spon')
        #fig, ax = plot_fft(freq_adapt_a2, coef_adapt_a2_mean_reliz, freq_max1=20, freq_max2 = 200, fig=fig, ax=ax, label='adapt')
        for i in range(2):
            for j in range(2):
        ...
        fig.suptitle('mean spectrum(30 realizations)'+title_a1+'\n[%.1f, %.1f]'%(mua_loca[0],mua_loca[1]))
        savetitle = title_a1.replace('\n','')
        fftfile = savetitle + '_fft_param%d'%(para) + '.png'
        fig.savefig(save_dir + fftfile)
        plt.close(fig)
        '''sens; tunning curve'''
        stim_amp = np.array([400,600,800])
        plt.figure(figsize=[8,7])
        dist_bin_plot = (dist_bin+(dist_bin[1]-dist_bin[0])*0.5)[:-1]
        for st in range(len(hz_loc_mean_reliz)):

                
            plt.errorbar(dist_bin_plot, hz_loc_mean_reliz[st], hz_loc_sem_reliz[st], ls='--', marker='o', c=clr[st], label = 'stim_amp: %.1f Hz'%(stim_amp[st]))
        
        plt.errorbar(dist_bin_plot, hz_loc_spon_mean_reliz, hz_loc_spon_sem_reliz, ls='--', marker='o', c=clr[st+1], label = 'spontaneous')
        ...
        savetitle = title_a1.replace('\n','')
        tunecvfile = savetitle+'_tunecv'+'_param%d'%para+'.png'
        plt.savefig(save_dir + tunecvfile)
        plt.close()
        '''sens; firing rate-time'''
        plt.figure(figsize=[8,6])
        t_plot = np.arange(400) - 100
        
        for st in range(stim_amp.shape[0]):
        
            plt.plot(t_plot, hz_t_mean_reliz[st], ls='--', c=clr[st], label = 'stim_amp: %.1f Hz'%(stim_amp[st]))
            plt.fill_between(t_plot, hz_t_mean_reliz[st]-hz_t_sem_reliz[st], hz_t_mean_reliz[st]+hz_t_sem_reliz[st], \
        ...
        savetitle = title_a1.replace('\n','')
        tempfile = savetitle+'_temp'+'_param%d'%para+'.png'
        plt.savefig(save_dir + tempfile)
        plt.close()        
        
        '''asso; fft'''
        #title = title + '_eier%.3f_iier%.3f\n'%(data.param.ie_r_e1, data.param.ie_r_i1)
        pkf_spon = find_peakF(coef_spon_a2_mean_reliz, freq_spon_a2, 3)#freq_spon[1:][coef_spon_mean[1:].argmax()]
        pkf_adapt = find_peakF(coef_adapt_a2_mean_reliz, freq_adapt_a2, 3)#freq_adapt[1:][coef_adapt_mean[1:].argmax()]
        title_a2 = title + '_a2_hz%.2f_pkfspon%.2f_pkfadpt%.2f'%(spon_rate_2,pkf_spon,pkf_adapt)

        '''fft'''
        fig, ax = plt.subplots(2,2,figsize=[9,9])
        fig, ax = plot_fft(freq_spon_a2, coef_spon_a2_mean_reliz, freq_max1=20, freq_max2 = 200, fig=fig, ax=ax, label='spon')
        fig, ax = plot_fft(freq_adapt_a2, coef_adapt_a2_mean_reliz, freq_max1=20, freq_max2 = 200, fig=fig, ax=ax, label='adapt')
        for i in range(2):
            for j in range(2):
                ax[i,j].plot([3,3],[0,6],'r--',lw=0.5)
        ...
        fig.suptitle('mean spectrum(30 realizations)'+title_a2+'\n[%.1f, %.1f]'%(mua_loca[0],mua_loca[1]))
        savetitle = title_a2.replace('\n','')
        fftfile = savetitle + '_fft_param%d'%(para) + '.png'
        fig.savefig(save_dir + fftfile)
        plt.close(fig)
