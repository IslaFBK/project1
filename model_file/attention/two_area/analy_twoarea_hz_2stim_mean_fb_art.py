...
# -*- coding: utf-8 -*-
"""
Created on Wed Mar  3 13:09:14 2021
...
import shutil
#%%
fftplot = 0; getfano = 0; get_nscorr = 1
get_TunningCurve = 0; get_HzTemp = 0


save_dir = 'mean_results/'
data_analy_file = 'data_anly_nc'#data_anly' #data_anly_fano data_anly_fano10hz
...
#datapath = '/import/headnode1/shni2598/brian2/NeuroNet_brian/model_file/attention/two_area/data/twoarea_hz/raw_data/'
datapath = 'raw_data/'
...
if get_HzTemp:
    ...
    hz_loc_elec_mean = np.zeros([n_StimAmp*2, data_anly.hz_loc_elec.shape[1], repeat])
    hz_t_mean[:] = np.nan
...
for i in range(0, 50):
    ...
    for loop_num in range(i*repeat, (i+1)*repeat):
        ...
        spon_rate[loop_num%repeat, 2] = data_anly.adapt_rate2
        if getfano:
    ...
    title += '_1hz%.2f_1adphz%.2f_2hz%.2f'%(spon_rate_mean[0], spon_rate_mean[2], spon_rate_mean[1])
