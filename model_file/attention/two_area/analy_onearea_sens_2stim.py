...
# -*- coding: utf-8 -*-
"""
Created on Mon Feb 15 00:08:25 2021
...
'''firing rate time'''
stim_amp_1 = 400
stim_amp_2 = np.array([200,400,600])#400

mua_loca_1 = [0, 0]
mua_range_1 = 5
mua_neuron_1 = cn.findnearbyneuron.findnearbyneuron(data.a1.param.e_lattice, mua_loca_1, mua_range_1, data.a1.param.width)
window = 5
dura_onoff = data.a1.param.stim2.stim_on.copy()
dura_onoff[:,0] -= 100
dura_onoff[:,1] += 100
hz_t_1 = fra.firing_rate_time_multi(data.a1.ge, mua_neuron_1, dura_onoff, window=window, n_neu_all=data.a1.param.Ne)
data_anly.hz_t_1 = hz_t_1
...
dura_onoff[:,0] -= 100
dura_onoff[:,1] += 100
hz_t_2 = fra.firing_rate_time_multi(data.a1.ge, mua_neuron_1, dura_onoff, window=window, n_neu_all=data.a1.param.Ne)
data_anly.hz_t_2 = hz_t_2


plt.figure(figsize=[8,6])
t_plot = np.arange(dura_onoff[0,1] - dura_onoff[0,0]) - 100

for st in range(stim_amp_2.shape[0]):

    hz_t_noatt_mean_1 = hz_t_1[st*30:st*30+30,:].mean(0)
    hz_t_noatt_mean_2 = hz_t_2[st*30:st*30+30,:].mean(0)
    #hz_t_noatt_std = hz_t[st*30:st*30+30,:].std(0)
    hz_t_noatt_sem_1 = scipy.stats.sem(hz_t_1[st*30:st*30+30,:])#.std(0)
    hz_t_noatt_sem_2 = scipy.stats.sem(hz_t_2[st*30:st*30+30,:])
    ...
    plt.plot(t_plot, hz_t_noatt_mean_1, ls='-', c=clr[st], label = 'loc_1; stim_1: %.1f Hz; stim_2: %.1f Hz'%(stim_amp_1,stim_amp_2[st]))
    plt.fill_between(t_plot, hz_t_noatt_mean_1-hz_t_noatt_sem_1,hz_t_noatt_mean_1+hz_t_noatt_sem_1, \
                     ls='-', facecolor=clr[st], edgecolor=clr[st], alpha=0.2)
    plt.plot(t_plot, hz_t_noatt_mean_2, ls='--', c=clr[st], label = 'loc_2; stim_1: %.1f Hz; stim_2: %.1f Hz'%(stim_amp_1,stim_amp_2[st]))
    plt.fill_between(t_plot, hz_t_noatt_mean_2-hz_t_noatt_sem_2,hz_t_noatt_mean_2+hz_t_noatt_sem_2, \
                     ls='--', facecolor=clr[st], edgecolor=clr[st], alpha=0.2)
...
start_time = data.a1.param.stim2.stim_on[first_stim,0] - 400
end_time = data.a1.param.stim2.stim_on[last_stim,0] + 400
...
stim_on_off = data.a1.param.stim2.stim_on-start_time
