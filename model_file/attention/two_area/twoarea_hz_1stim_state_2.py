...
# -*- coding: utf-8 -*-
"""
Created on Mon Apr 12 12:56:46 2021
...
'''
low firing rate in sensory area
increase stim_scale_cls.separate_dura = np.array([300,600]) a little bit
one stimulus
for scale_w_21_e in np.arange(0.35, 0.45, 0.02):#np.arange(0.8,1.21,0.05):#[1]:#np.arange(0.8,1.21,0.05):[0.3]
for scale_w_21_i in scale_w_21_e*np.array([0.95,1,1.05])
for tau_p_d_e2_e1 in [13,15,17]:#np.arange(7,14.1,1):
for tau_p_d_e2_i1 in [13,15,17]

adapt_logistic.get_adaptation
'''
...
for ie_r_i1 in np.arange(1.52,1.521,0.02):#[1.40]:
    for ie_r_i2 in np.arange(1.04, 1.071, 0.01):
        #for w_extnl_ in [10]:
        for delta_gk_1 in [3.5]:#np.linspace(7,15,9)]]:
            for delta_gk_2 in [16]:
                for new_delta_gk_2 in np.arange(0.5, 2.1, 0.5):#[2]:#np.arange(0.15,0.31,0.05)*delta_gk_:#np.linspace(1,5,5):
                                                        ...
                                                        for tau_p_d_e2_i1 in [15]:#np.arange(7,14.1,1):#[10]:
                                                            for peak_p_e1_e2 in [0.4]:#np.arange(0.3,0.451,0.05):
                                                                for peak_p_e1_i2 in [0.4]:#np.arange(0.25,0.401,0.05):
                                                                    for peak_p_e2_e1 in [0.1,0.12,0.14]:#np.arange(0.10):#[0.15]: # 0.18
...
adapt_value = adapt_logistic.get_adaptation(base_amp = delta_gk_2, \
    max_decrease = [delta_gk_2-new_delta_gk_2], rang = [chg_adapt_range, chg_adapt_range], \
        sharpness =[1.7], position=[chg_adapt_loca], n_side=int(round((ijwd2.Ne)**0.5)), width=ijwd2.width)
