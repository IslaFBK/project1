...
# -*- coding: utf-8 -*-
"""
Created on Thu Feb 11 17:50:39 2021
...
if loop_num%25 == 0:
    ...
    savecwtfile =True; 
    saveMSDfile = True; savecfcfile = True
    saveani_adapt = True; saveani_stim = True;
...
'''MSD alpha'''
data.a1.ge.get_MSD(start_time=5000, end_time=20000, n_neuron=data.a1.param.Ne, window = 10, jump_interval=np.array([15]), fit_stableDist='pylevy')
...
for mua_loc_ind in range(len(mua_loca_all)):
    ...
    fftfile = savetitle+'_loc%d_%d'%(mua_loc_ind, loop_num)+'.png'
    if savefftfile == True and mua_loc_ind == 0:
        fig.savefig(fftfile)
    #%%
    ...
    ratefile = savetitle+'_loc%d_%d'%(mua_loc_ind, loop_num)+'.png'
    if saveratefile == True and mua_loc_ind == 0:
    ...
    cwtfile = savetitle+'_loc%d_%d'%(mua_loc_ind, loop_num)+'.png'
    if savecwtfile and mua_loc_ind == 0:
        fig.savefig(cwtfile)
    #%%
    ...
    data.a1.ge.get_MSD(start_time=5000, end_time=20000, n_neuron = data.a1.param.Ne, \
                       ...
                       window = 10, jump_interval=np.logspace(0.1,3,10), fit_stableDist='pylevy')
    ...
    kslop = data.a1.ge.MSD.MSD[0]/data.a1.ge.MSD.jump_interval[0]
    y_unislop = kslop * np.logspace(0.1,1.5,10)
    ax1.loglog(np.logspace(0.1,1.5,10), y_unislop, '--', label='k=1')
    ax1.legend()
    ...
    alpha = data.a1.ge.MSD.stableDist_param[2,0]
    titleMSD = title + '_alpha%.2f'%alpha
    ax1.set_title(titleMSD+'\n'+'[%.1f, %.1f]'%(mua_loca[0],mua_loca[1]))
    
    savetitle = titleMSD.replace('\n','')
    MSDfile = savetitle+'_loc%d_%d'%(mua_loc_ind, loop_num)+'.png'
    if saveMSDfile and mua_loc_ind == 0:
        fig.savefig(MSDfile)
    ...
    if savecfcfile:
        ...
        data_anly.cfc.phaseBand = phaseBand
        data_anly.cfc.ampBand = ampBand
        data_anly.cfc.MI_raw = MI_raw
        data_anly.cfc.MI_surr = MI_surr
        #%%
        ...
        cfcfile = savetitle+'_loc%d_%d'%(mua_loc_ind, loop_num)+'.png'
        if mua_loc_ind == 0:
            fig.savefig(cfcfile)
    ...
    moviefile = savetitle+'_adapt_loc%d_%d'%(mua_loc_ind, loop_num)+'.mp4'
    if saveani_adapt and mua_loc_ind == 0:
        ani.save(moviefile)
        pass
    ...
    moviefile = savetitle+'_stim_loc%d_%d'%(mua_loc_ind, loop_num)+'.mp4'
    if saveani_stim and mua_loc_ind == 0:
        ani.save(moviefile)
        pass
#%%
    data_anly.save(data_anly.class2dict(), datapath+'data_anly_electrode_loc%d_%d.file'%(mua_loc_ind,loop_num))
