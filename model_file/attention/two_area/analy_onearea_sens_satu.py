...
# -*- coding: utf-8 -*-
"""
Created on Sun Mar 14 01:23:09 2021
...
if analy_type == 'sens': # weak feedback
    title = 'hz_erie%.2f_irie%.2f_wii%.1f'%(data.param.ie_r_e1, \
                                               data.param.ie_r_i1, data.a1.param.w_ii_mean)    

    #%%
n_StimAmp = 6
n_perStimAmp = 30
stim_amp = 200*(1+np.arange(n_StimAmp))
#%%
'''spontanous rate'''
dt = 1/10000;
...
if get_TunningCurve:
    ...
    #N_stim = int(round(data.a1.param.stim1.stim_amp_scale.shape[0]))
    N_stim = n_StimAmp*n_perStimAmp
...
if get_HzTemp:
    ...
    for mua_loca_1 in [[0,0]]:#,[-32,-32]]:
        ...
        hz_t = hz_t.reshape(n_StimAmp, n_perStimAmp, -1)
    ...
    data.a1.ge.get_sparse_spk_matrix([data.a1.param.Ne, simu_time_tot*10])
    ## stim
    hz_loc_elec = fra.tuning_curve(data.a1.ge.spk_matrix, data.a1.param.stim.stim_on, n_in_bin)
    
    data_anly.hz_loc_elec = hz_loc_elec.copy()
    
    hz_loc_elec = hz_loc_elec.reshape(n_StimAmp, n_perStimAmp, -1)
    ...
    hz_loc_elec_mean_tmp = np.zeros([n_StimAmp*1+1, 2])
    ...
    hz_loc_elec_sem_tmp = np.zeros([n_StimAmp*1+1, 2])
...
if firing_rate_long:
    ...
    start_time = data.a1.param.stim.stim_on[n_StimAmp*n_perStimAmp,0] - sti_on
    end_time = data.a1.param.stim.stim_on[n_StimAmp*n_perStimAmp+5,0] + 500 
    ...
    '''no attention'''
    start_time = data.a1.param.stim.stim_on[0,0] - sti_on
    end_time = data.a1.param.stim.stim_on[5,0] + 500 
    # fra.get_spike_rate(data.a1.ge, start_time, end_time, indiv_rate = True, popu_rate = False, \
    #                    sample_interval = 1, n_neuron = data.a1.param.Ne, window = 10, dt = 0.1, reshape_indiv_rate = False, save_results_to_input = False):
    data.a1.ge.get_spike_rate(start_time,end_time,window=window,n_neuron = data.a1.param.Ne,reshape_indiv_rate = False)
...
if get_ani:
    ...
    start_time = data.a1.param.stim.stim_on[first_stim,0] - 1000
    end_time = data.a1.param.stim.stim_on[last_stim,0] 
    ...
    first_stim = 29; last_stim = 30
    start_time = data.a1.param.stim.stim_on[first_stim,0] - 300
    end_time = data.a1.param.stim.stim_on[last_stim,0] + 500
    ...
    #stim = [[[[31.5,31.5]], [stim_on_off], [[6]*stim_on_off.shape[0]]],None]
    stim = [[[[31.5,31.5]], [stim_on_off], [[6]*stim_on_off.shape[0]]]]
