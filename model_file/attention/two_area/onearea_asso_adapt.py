...
# -*- coding: utf-8 -*-
"""
Created on Mon Apr  5 17:29:18 2021

@author: shni2598
"""


'''change spontaneous state and adaptation modulation'''
...
for ie_r_e in [2.76*6.5/5.8]:# np.linspace(3.07,3.13,7):#np.arange(2.5,4.0,0.04):#[3.1]:#np.linspace(3.10,3.45,6):#np.linspace(1.4,1.6,10):#[1.5]: #np.linspace(1.,1.2,5):
    for ie_r_e1 in np.arange(1,1.07,0.02):#[1.03]:#np.arange(1.4,1.611,0.02):
        for w_ee_ in [None]:#[4]:
            for w_ie_ in [115]:#[23.12]:#[17]:#np.linspace(17,23,7):#np.linspace(1.077,1.277,10):#[1.177]:#np.linspace(1,1,1):
                for ie_r_i in [2.450*6.5/5.8]:#[2.719]:#[2.817]:#[2.786]:#np.linspace(2.5,3.0,15):#np.linspace(1.156-0.08,1.156+0.08,15):#np.linspace(1.156-0.175,1.156+0.175,31):#np.linspace(1.157,1.357,10):#[1.257]: #np.linspace(0.95,1.1,5):
                    for ie_r_i1 in ie_r_e1*np.arange(0.9,1.11,0.02): #np.arange(0.9,1.11,0.01)
                            ...
                            for w_ei_ in [None]:#[6.35]:
                                for w_extnl_ in [10]:
                                    for delta_gk_ in [16]:#np.linspace(7,15,9)]]:
                                        for new_delta_gk_ in [0,1,2,3]:#[delta_gk_/5]:#np.arange(0.15,0.31,0.05)*delta_gk_:#np.linspace(1,5,5):
...
# adapt_value = adapt_gaussian.get_adaptation(base_amp = delta_gk_, \
#     max_decrease = [delta_gk_-new_delta_gk_], sig=[chg_adapt_range], position=[chg_adapt_loca], n_side=int(round((ijwd1.Ne)**0.5)), width=ijwd1.width)

adapt_value = adapt_logistic.get_adaptation(base_amp = delta_gk_, \
    max_decrease = [delta_gk_-new_delta_gk_], rang = [8, 8], sharpness =[1.7], position=[chg_adapt_loca], n_side=int(round((ijwd1.Ne)**0.5)), width=ijwd1.width)
...
#seed(10)
simu_time_tot = 30000*ms #(stim_scale_cls.stim_on[-1,1] + 500)*ms

simu_time1 = 10000*ms #simu_time_tot#20000*ms#2000*ms
simu_time2 = simu_time_tot - simu_time1 #15000*ms
simu_time3 = 5000*ms
simu_time4 = 15000*ms
#simu_time5 = 5000*ms
...
group_e_1.delta_gk[:] = adapt_value*nS
