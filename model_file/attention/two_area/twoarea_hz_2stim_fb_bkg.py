...
# -*- coding: utf-8 -*-
"""
Created on Thu Mar 25 18:28:43 2021

@author: shni2598
"""

'''
increase background input when stimulus onset ; bkg_rates : Hz
'''
...
for ie_r_i1 in [1.48]:#np.arange(1.40,1.521,0.02):#[1.40]:
    #for w_extnl_ in [10]:
    for bkg_rate_e in [100,200,300]:
        for bkg_rate_i in bkg_rate_e*np.array([1]): #bkg_rate_e*np.array([0.3,0.5,0.7,0.9])
                    ...
                    for new_delta_gk_2 in [2]:#np.arange(0.15,0.31,0.05)*delta_gk_:#np.linspace(1,5,5):
                        for tau_s_di_ in np.linspace(4.4,4.4,1):
                            for tau_s_de_ in np.linspace(5.,5.,1):
                                ...
                                for scale_w_12_e in [2.8]:#np.arange(2, 2.81, 0.2):#np.arange(0.8,1.21,0.05):[2.5]
                                                ...
                                                for tau_p_d_e1_e2 in [5]:
                                                    for tau_p_d_e1_i2 in [5]:
                                                        for tau_p_d_e2_e1 in [15]:#np.arange(7,14.1,1):
                                                            for tau_p_d_e2_i1 in [15]:#np.arange(7,14.1,1):#[10]:
                                                                for peak_p_e1_e2 in [0.4]:#np.arange(0.3,0.451,0.05):
                                                                    for peak_p_e1_i2 in [0.4]:#np.arange(0.25,0.401,0.05):
                                                                        for peak_p_e2_e1 in [0.1]:#[0.1,0.12,0.14]:#np.arange(0.10):#[0.15]: # 0.18
                                                                            for peak_p_e2_i1 in [0.1]*repeat:#peak_p_e2_e1*np.repeat(np.arange(0.9,1.1,0.03),repeat):#:[0.15]*repeat: # 0.18
...
scale_bkg = stim_scale_cls.scale_stim.copy()
scale_bkg[0:int(round(stim_scale_cls.stim_on[n_StimAmp*n_perStimAmp,0]/stim_scale_cls.dt_stim))] = 0

# att_point = int(round((stim_scale_cls.stim_on[n_StimAmp*n_perStimAmp-1,1] + int(round(inter_time/2)))/stim_scale_cls.dt_stim))
# scale_bkg[0:att_point] = 0
# scale_bkg[att_point:] = 1

scale_bkg_1 = TimedArray(scale_bkg, dt=10*ms)
#%%
...
stim_amp_scale = np.ones(n_StimAmp*n_perStimAmp)
for i in range(n_StimAmp):
    stim_amp_scale[i*n_perStimAmp:i*n_perStimAmp+n_perStimAmp] = 0#2**i
...
posi_stim_e1 = NeuronGroup(ijwd1.Ne, \
                        '''rates =  bkg_rates*scale_bkg_1(t) + stim_1*scale_1(t) + stim_2*scale_2(t) : Hz
                        bkg_rates : Hz
                        stim_1 : Hz
                        stim_2 : Hz
                        ''', threshold='rand()<rates*dt')

posi_stim_e1.bkg_rates = bkg_rate_e*Hz
...
posi_stim_i1 = NeuronGroup(ijwd1.Ni, \
                        '''rates =  bkg_rates*scale_bkg_1(t) + stim*scale_1(t) : Hz
                        bkg_rates : Hz
                        stim : Hz
                        ''', threshold='rand()<rates*dt')

posi_stim_i1.bkg_rates = bkg_rate_i*Hz
posi_stim_i1.stim = psti.input_spkrate(maxrate = [0], sig=[6], position=[[0, 0]], n_side=32, width=64)*Hz
...
param_all = {'delta_gk_1':delta_gk_1,
         ...
         'ie_r_i2': ie_r_i2,
         't_ref': t_ref/ms,
         'bkg_rate_e':bkg_rate_e,
         'bkg_rate_i':bkg_rate_i}
