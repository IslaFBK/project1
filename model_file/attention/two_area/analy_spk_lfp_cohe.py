...
# -*- coding: utf-8 -*-
"""
Created on Wed Jun 30 15:26:02 2021
...
analy_type = 'fffbrg'
datapath = data_dir
#datapath = '/import/headnode1/shni2598/brian2/NeuroNet_brian/model_file/attention/two_area/data/twoarea_hz/state/param1/stim2/longstim/lfp_cohe/raw_data/'
sys_argv = int(sys.argv[1])
loop_num = sys_argv #rep_ind*20 + ie_num
good_dir = 'good/'
goodsize_dir = 'good_size/'

savefile_name = 'data_anly_coh' #'data_anly' data_anly_temp data_anly_nc_normal
save_apd = ''
...
spk_loca = [-32, -32]
spk_range = 2 
spk_neuron_noatt = cn.findnearbyneuron.findnearbyneuron(data.a1.param.e_lattice, spk_loca, spk_range, data.a1.param.width)

spk_loca = [0, 0]
spk_range = 2 
spk_neuron_att = cn.findnearbyneuron.findnearbyneuron(data.a1.param.e_lattice, spk_loca, spk_range, data.a1.param.width)
...
spk_mat_noatt = data.a1.ge.spk_matrix[spk_neuron_noatt]
spk_mat_att = data.a1.ge.spk_matrix[spk_neuron_att]
...
for st in range(n_StimAmp):
    ...
    dura_noatt = data.a1.param.stim1.stim_on[(st+n_StimAmp)*n_perStimAmp:(st+n_StimAmp+1)*n_perStimAmp].copy()
    dura_att = data.a1.param.stim1.stim_on[(st+n_StimAmp)*n_perStimAmp:(st+n_StimAmp+1)*n_perStimAmp].copy()
    

    R_noatt = spk_cohe.spk_lfp_coherence(spk_mat_noatt, data.a1.ge.LFP[1], dura_noatt, discard_init = 200, hfwin_lfp_seg=150, dt = 0.1)
    R_att = spk_cohe.spk_lfp_coherence(spk_mat_att, data.a1.ge.LFP[0], dura_att, discard_init = 200, hfwin_lfp_seg=150, dt = 0.1)
    ...
    if n_StimAmp == 1:
        ...
        ax[1].loglog(R_noatt.freq[1:plt_len], R_noatt.staLFP_pw[1:plt_len], label='staLFP_pw,noatt;%dHz'%stim_amp[st])
        ax[1].loglog(R_att.freq[1:plt_len], R_att.staLFP_pw[1:plt_len], label='staLFP_pw,att;%dHz'%stim_amp[st])
    
        ax[2].plot(np.arange(R_noatt.staLFP.shape[0])/10, R_noatt.staLFP, label='staLFP,noatt;%dHz'%stim_amp[st])
        ax[2].plot(np.arange(R_att.staLFP.shape[0])/10, R_att.staLFP, label='staLFP,att;%dHz'%stim_amp[st])
        ...
        ax[1,st].loglog(R_noatt.freq[1:plt_len], R_noatt.staLFP_pw[1:plt_len], label='staLFP_pw,noatt;%dHz'%stim_amp[st])
        ax[1,st].loglog(R_att.freq[1:plt_len], R_att.staLFP_pw[1:plt_len], label='staLFP_pw,att;%dHz'%stim_amp[st])
        
        ax[2,st].plot(np.arange(R_noatt.staLFP.shape[0])/10, R_noatt.staLFP, label='staLFP,noatt;%dHz'%stim_amp[st])
        ax[2,st].plot(np.arange(R_att.staLFP.shape[0])/10, R_att.staLFP, label='staLFP,att;%dHz'%stim_amp[st])
