...
# -*- coding: utf-8 -*-
"""
Created on Mon Mar  1 14:30:45 2021
...
savefile_name = 'data_anly_nc' #'data_anly'
...
if get_HzTemp:
    ...
    for i in range(len(n_in_bin)):
        ax[1].errorbar(np.arange(stim_amp.shape[0]), hz_loc_elec_mean[:n_StimAmp,i], hz_loc_elec_sem[:n_StimAmp,i], \
                     fmt='--', c=clr[i], marker='o', label='no attention, loc: [%.1f,%.1f]'%(mua_loca[i][0],mua_loca[i][1]))
        ax[1].errorbar(np.arange(stim_amp.shape[0]), hz_loc_elec_mean[n_StimAmp:2*n_StimAmp,i], hz_loc_elec_sem[n_StimAmp:2*n_StimAmp,i], \
                     fmt='-', c=clr[i], marker='o', label='attention, loc: [%.1f,%.1f]'%(mua_loca[i][0],mua_loca[i][1]))

    ax[1].legend()
    ax[1].xaxis.set_ticks([i for i in range(len(stim_amp))])
...
#data_anly.save(data_anly.class2dict(), datapath+'data_anly_fano%dhz%d.file'%(bin_count_interval_hz,loop_num))
