...
# -*- coding: utf-8 -*-
"""
Created on Sun Jan 24 21:18:19 2021
...
for w_extnl_ in [10]:
    for delta_gk_1 in [3.5]:#np.linspace(7,15,9)]]:
        for delta_gk_2 in [16]:
            for new_delta_gk_2 in [3]:#np.arange(0.15,0.31,0.05)*delta_gk_:#np.linspace(1,5,5):
                for tau_s_di_ in np.linspace(4.4,4.4,1):
                    for tau_s_de_ in np.linspace(5.,5.,1):
                        for tau_s_r_ in [1]:#np.linspace(1,1,1):
                            for scale_w_12_e in np.array([1.2]):#np.arange(0.8,1.21,0.05):
                                for scale_w_12_i in np.array([1.2]):#np.arange(0.8,1.21,0.05):#[scale_w_12_e]:#0.2*np.arange(0.8,1.21,0.05):
                                    for scale_w_21_e in [1.15]:#np.arange(0.8,1.21,0.05):#[1]:#np.arange(0.8,1.21,0.05):
                                        for scale_w_21_i in [1.10]:#np.arange(0.8,1.21,0.05):#[1]:#[scale_w_21_e]:#0.35*np.arange(0.8,1.21,0.05):
                                                        ...
                                                        for tau_p_d_e2_i1 in [10]:
                                                            for peak_p_e1_e2 in [0.45]:#np.arange(0.3,0.451,0.05):
                                                                for peak_p_e1_i2 in [0.45]:#np.arange(0.25,0.401,0.05):
                                                                    for peak_p_e2_e1 in [0.4]:
                                                                        for peak_p_e2_i1 in [0.4]*repeat:
...
ijwd2.w_ee_mean = find_w_e(w_ie_2, num_ie, num_ee, ie_r_e*ie_r_e1)#find_w_e(w_ie_, num_ie, num_ee, ie_r_e)
ijwd2.w_ei_mean = find_w_e(w_ii_2, num_ii, num_ei, ie_r_i*ie_r_i1)
...
ijwd_inter.w_e2_e1_mean = 7*scale_w_21_e*1; ijwd_inter.w_e2_i1_mean = 7*scale_w_21_i*1
...
simu_time1 = (stim_scale_cls.stim_on[99,1] + 2000)*ms
