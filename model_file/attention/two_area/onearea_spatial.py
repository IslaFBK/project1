...
# -*- coding: utf-8 -*-
"""
Created on Sun Mar 21 19:37:02 2021
...
for ie_r_e in [0.95]:# np.linspace(3.07,3.13,7):#np.arange(2.5,4.0,0.04):#[3.1]:#np.linspace(3.10,3.45,6):#np.linspace(1.4,1.6,10):#[1.5]: #np.linspace(1.,1.2,5):
    for ie_r_e1 in [1]:#np.arange(1,1,1):
        for w_ee_ in [None]:#[4]:
            for w_ie_ in [100]:#[23.12]:#[17]:#np.linspace(17,23,7):#np.linspace(1.077,1.277,10):#[1.177]:#np.linspace(1,1,1):
                for ie_r_i in [1]:#[2.719]:#[2.817]:#[2.786]:#np.linspace(2.5,3.0,15):#np.linspace(1.156-0.08,1.156+0.08,15):#np.linspace(1.156-0.175,1.156+0.175,31):#np.linspace(1.157,1.357,10):#[1.257]: #np.linspace(0.95,1.1,5):
                    for ie_r_i1 in ie_r_e1*np.arange(1,1.1,1): #np.arange(0.9,1.11,0.01)
                        for w_ii_ in [100]:#[100]:#[25]:#np.linspace(20,27,8):#np.arange(1.4,1.81,0.1)]]:#np.linspace(1.,1.,1):
                            for w_ei_ in [None]:#[6.35]:
                                for w_extnl_ in [10]:
                                    for delta_gk_ in [5]:#np.linspace(7,15,9)]]:
                                        for new_delta_gk_ in [delta_gk_/5]:#np.arange(0.15,0.31,0.05)*delta_gk_:#np.linspace(1,5,5):
                                            for tau_s_di_ in np.linspace(8,8,1):
...
ijwd1.decay_p_ee = 8#8*0.8 #* scale_d_p_ee#scale_d_p # decay constant of e to e connection probability as distance increases
ijwd1.decay_p_ei = 8#10*0.82 #* scale_d_p_ei# scale_d_p # decay constant of e to i connection probability as distance increases
ijwd1.decay_p_ie = 8#20*0.86 #* scale_d_p_ie#scale_d_p_i#* scale_d_p # decay constant of i to e connection probability as distance increases
ijwd1.decay_p_ii = 8#20*0.86 #* scale_d_p_ii#* scale_d_p # decay constant of i to i connection probability as distance increases
ijwd1.delay = [0.5,2.5]
num_ee = 150; num_ei = 300; num_ie = 75; num_ii = 75
...
stim_amp_scale = np.ones(180)
for i in range(6):
    stim_amp_scale[i*30:i*30+30] = i+1
...
posi_stim_e1.stim = psti.input_spkrate(maxrate = [200], sig=[6], position=[[0, 0]], n_side=64, width=64)*Hz
...
posi_stim_i1.stim = psti.input_spkrate(maxrate = [200], sig=[6], position=[[0, 0]], n_side=32, width=64)*Hz
...
syn_ei_1.w = ijwd1.w_ei*nsiemens * scale_ei #tau_s_de_ #5*nS
syn_ie_1.w = ijwd1.w_ie*nsiemens * scale_ie#tau_s_di_#25*nS
...
#seed(10)
simu_time_tot = 1000*ms#(stim_scale_cls.stim_on[-1,1] + 500)*ms

simu_time1 = simu_time_tot#20000*ms#2000*ms
simu_time2 = 15000*ms
simu_time3 = 5000*ms
simu_time4 = 15000*ms
#simu_time5 = 5000*ms
...
data_ = {'datetime':now.strftime("%Y-%m-%d %H:%M:%S"), 'dt':0.1, 'loop_num':loop_num, 'data_dir': os.getcwd(),
        'param':param_all,
        'a1':{'param':param_a1,
...
ani = fra.show_pattern(spkrate1=data.a1.ge.spk_rate.spk_rate, spkrate2=None, \
                                        frames = frames, start_time = start_time, interval_movie=15, anititle='equal coupling spatial range',stim=None, adpt=None)
