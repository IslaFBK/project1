...
# -*- coding: utf-8 -*-
"""
Created on Tue May  4 21:27:22 2021
...
ie_r_e1_list = np.concatenate((ie_r_e1_list,ie_r_e1_list[::-1]))
...
for ie_r_e in [2.76*6.5/5.8]:# np.linspace(3.07,3.13,7):#np.arange(2.5,4.0,0.04):#[3.1]:#np.linspace(3.10,3.45,6):#np.linspace(1.4,1.6,10):#[1.5]: #np.linspace(1.,1.2,5):
    for ie_r_e1 in [1.56]:#np.arange(1.5,1.71,0.02):
        for w_ee_ in [None]:#[4]:
...
#syn_ie_1.w = ijwd1.w_ie*ie_r_e1_list[0]*nsiemens
#%%
...
stim_amp_scale = np.ones(90)
for i in range(3):
    stim_amp_scale[i*30:i*30+30] = i+2
...
param_a1 = {**param_a1, 'stim':mydata.mydata.class2dict(stim_scale_cls)}
...
stim_data.maxrate_stim = maxrate_stim
stim_data.scale_stim = scale_stim

param_a1 = {**param_a1, 'stim':stim_data.class2dict(stim_data)}
...
posi_stim_e1.stim = psti.input_spkrate(maxrate = [maxrate_stim,maxrate_stim], sig=[6,6], position=[[0, 0],[-32, -32]], n_side=64, width=64)*Hz
...
simu_time_init = 5000*ms
simu_time_trial = 5000*ms
simu_time_tot = simu_time_init + simu_time_trial*len(ie_r_e1_list)

syn_ie_1.w = ijwd1.w_ie*ie_r_e1_list[0]*nsiemens
net.run(simu_time_init, profile=False)
   
for ii in range(len(ie_r_e1_list)):
    syn_ie_1.w = ijwd1.w_ie*ie_r_e1_list[ii]*nsiemens
    net.run(simu_time_trial, profile=False) 
