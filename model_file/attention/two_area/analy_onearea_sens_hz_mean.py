...
# -*- coding: utf-8 -*-
"""
Created on Mon Jan 11 17:29:27 2021
...
n_amp_stim = 5
n_per_amp = 10
for ie_ind in range(n_ie):
    hz_loc = np.zeros([N_stim,len(dist_bin)-1,len(fr_bin)])
    spon_rate1 = 0
    hz_t = np.zeros([N_stim, 400])
    hz_loc_spon = np.zeros([len(dist_bin)-1])
    ...
    for loop_num in range(ie_ind*trial_per_ie, ie_ind*trial_per_ie+trial_per_ie):
        ...
        hz_loc_spon += data_analy.hz_spon
    ...
    for b in range(1):
        ...
        for i in range(n_amp_stim):
            
            #plt.plot(dist_bin_plot, hz_loc_mean[i]/(fr_bin[b]/1000), '-o', label = 'stim_amp: %.1f Hz'%(stim_amp[i]))
            plt.plot(dist_bin_plot, hz_loc_mean[i], '-o', label = 'stim_amp: %.1f Hz'%(stim_amp[i]))
        plt.plot(dist_bin_plot, hz_loc_spon, '-o', label = 'spontaneous')
        title = "eier:%.3f_iier:%.3f_1hz:%.2f"%(ie_r_e[ie_ind//ie_r_i.shape[0]], ie_r_e[ie_ind//ie_r_i.shape[0]]*ie_r_i[ie_ind%ie_r_i.shape[0]], spon_rate1)
        ...
        plt.ylabel('Hz')
        plt.legend()
        plt.savefig(save_dir+'stime_'+title.replace(':','')+'_countbin%.0f'%fr_bin[b]+'_%d'%ie_ind+'.png')
    
    hz_t_mean = hz_t.reshape(n_amp_stim,n_per_amp,-1).mean(1)
    plt.figure(figsize=[8,6])
    for i in range(n_amp_stim):
    ...
    plt.title(title)
    plt.legend()
    plt.savefig(save_dir+'stime_'+title.replace(':','')+'_temp'+'_%d'%ie_ind+'.png')
    ...
    #data_benmk.save(data_benmk.class2dict(), datapath+'data_benmk%d.file'%ie_ind)
