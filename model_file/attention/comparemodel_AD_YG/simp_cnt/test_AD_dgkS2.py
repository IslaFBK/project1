...
# -*- coding: utf-8 -*-
"""
Created on Wed Nov 11 17:07:05 2020
...
# -*- coding: utf-8 -*-
"""
Created on Wed Nov 11 16:45:05 2020
...
#%%
loop_num = -1
#e17.0_erie3.240_ii24.0_irie2.964_
#param_in = [[0.08,6.5]]
#repeat = 10
    ...
    for scale_d_p_ei in [1]:#np.linspace(0.85,1.15,13):#[1]
        for scale_d_p_ii in np.linspace(1,1,1):
            for scale_d_p_ie in np.linspace(1,1,1):#np.linspace(0.85,1.15,13):
                for num_ee in [260]:#np.linspace(220,290,8):#[260]:#np.linspace(125, 297, 10,dtype=int): 
                    for num_ei in [400]:#np.linspace(204, 466, 10, dtype=int):#[320]:               
                        for num_ie in [175]:#[260]:#np.linspace(246, 300, 6,dtype=int):#[221]:#np.linspace(156, 297, 10,dtype=int):
                            for num_ii in [260]:#np.linspace(220,300,9):#[260]:#np.linspace(135, 170,6,dtype=int):#[129]:#np.linspace(93, 177, 10,dtype=int):
                                for w_ee_ in [4.5]:
                                    for ie_r_e in np.linspace(3.10,3.2,3):#[3.15]:#np.linspace(2.7,3.2,11):#[3.24]:#np.arange(3.05,3.36,0.05):#np.linspace(1.4,1.6,10):#[1.5]: #np.linspace(1.,1.2,5):                                    
                                        for w_ie_ in [20]:#np.linspace(17,23,7):#np.linspace(1.077,1.277,10):#[1.177]:#np.linspace(1,1,1):
                                            for w_ei_ in np.linspace(4.5,4.5,1):#[4.438]:#np.linspace(2.5,3.0,15):#np.linspace(1.156-0.08,1.156+0.08,15):#np.linspace(1.156-0.175,1.156+0.175,31):#np.linspace(1.157,1.357,10):#[1.257]: #np.linspace(0.95,1.1,5):
                                                for w_ii_ in [20]:#np.linspace(20,27,8):#np.arange(1.4,1.81,0.1)]]:#np.linspace(1.,1.,1):
                                                    for ie_r_i in np.linspace(2.65,2.75,3):#[2.7]:#np.linspace(2.3,2.7,9):#np.arange(2.8,3.11,0.05):
                                                        for w_extnl_ in [1.5]:
                                                            for delta_gk_ in np.linspace(5,15,11):#[12]:#np.linspace(7,15,9)]]:
                                                                for new_delta_gk_ in [delta_gk_/5]:#np.arange(0.15,0.31,0.05)*delta_gk_:#np.linspace(1,5,5):
                                                                    for tau_s_di_ in [4.444]:#np.linspace(3,6, 7):
                                                                        for tau_s_de_ in [5]:#np.linspace(3.,6.,7):
...
ijwd.decay_p_ee = 7 #* scale_d_p_ee#scale_d_p # decay constant of e to e connection probability as distance increases
ijwd.decay_p_ei = 9 #* scale_d_p_ei# scale_d_p # decay constant of e to i connection probability as distance increases
ijwd.decay_p_ie = 20 #* scale_d_p_ie#scale_d_p_i#* scale_d_p # decay constant of i to e connection probability as distance increases
ijwd.decay_p_ii = 20 #* scale_d_p_ii#* scale_d_p # decay constant of i to i connection probability as distance increases

ijwd.mean_SynNumIn_ee = num_ee     ; # p = 0.08
ijwd.mean_SynNumIn_ei = num_ei #* 8/5     ; # p = 0.125
ijwd.mean_SynNumIn_ie = num_ie  #scale_d_p_i    ; # p = 0.2
ijwd.mean_SynNumIn_ii = num_ii# 221 * scale_d_p_i#* 8/5     ; # p = 0.25

ijwd.w_ee_mean = w_ee_ #find_w_e(w_ie_, num_ie, num_ee, ie_r_e)
ijwd.w_ei_mean = w_ei_ #find_w_e(w_ii_, num_ii, num_ei, ie_r_i)
ijwd.w_ie_mean = find_w_i(w_ee_, num_ee, num_ie, ie_r_e) #w_ie_
ijwd.w_ii_mean = find_w_i(w_ei_, num_ei, num_ii, ie_r_i) #w_ii_
        
ijwd.generate_ijw()
ijwd.generate_d_rand()

# ijwd.w_ee *= scale_ee_1#tau_s_de_scale_d_p_i
