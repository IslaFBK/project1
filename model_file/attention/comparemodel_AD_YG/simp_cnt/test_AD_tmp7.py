...
# -*- coding: utf-8 -*-
"""
Created on Wed Nov  4 17:15:44 2020
...
for scale_d_p_ee in [1]:#np.linspace(0.85,1.15,13):
    for scale_d_p_ei in [1]:#np.linspace(0.85,1.15,13):#[1]
        for scale_d_p_ii in [1]:#np.linspace(0.85,1.15,15):
            for scale_d_p_ie in [1]:#np.linspace(0.85,1.15,13):
                for num_ee in [200]:#np.linspace(125, 297, 10,dtype=int): 
                    for num_ei in [320]:#np.linspace(204, 466, 10, dtype=int):#[320]:               
                        for num_ii in [221]:#np.linspace(156, 297, 10,dtype=int):
                            for num_ie in [129]:#np.linspace(93, 177, 10,dtype=int):
                                for scale_ee_1 in [1.2]:#np.linspace(1.4,1.6,10):#[1.5]: #np.linspace(1.,1.2,5):
                                    for scale_ei_1 in [1.27]:#np.linspace(1.077,1.277,10):#[1.177]:#np.linspace(1,1,1):
                                        for scale_ie_1 in np.linspace(1.156-0.15,1.156+0.15,25):#np.linspace(1.156-0.175,1.156+0.175,31):#np.linspace(1.157,1.357,10):#[1.257]: #np.linspace(0.95,1.1,5):
                                            for scale_ii_1 in [1.0]:#np.arange(1.4,1.81,0.1)]]:#np.linspace(1.,1.,1):
                                                for w_extnl_ in [1.5]:
                                                    for delta_gk_ in np.linspace(8,16,9):#np.linspace(7,15,9)]]:
                                                        for new_delta_gk_ in [delta_gk_/4.5]:#np.arange(0.15,0.31,0.05)*delta_gk_:#np.linspace(1,5,5):
                                                            for tau_k_ in np.linspace(40,120, 9):
...
ijwd.decay_p_ee = 8 * 0.8 * scale_d_p_ee#scale_d_p # decay constant of e to e connection probability as distance increases
ijwd.decay_p_ei = 10 * 0.82 * scale_d_p_ei# scale_d_p # decay constant of e to i connection probability as distance increases
ijwd.decay_p_ie = 17.2 * scale_d_p_ie#scale_d_p_i#* scale_d_p # decay constant of i to e connection probability as distance increases
ijwd.decay_p_ii = 17.2 * scale_d_p_ii#* scale_d_p # decay constant of i to i connection probability as distance increases
