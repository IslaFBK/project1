...
# -*- coding: utf-8 -*-
"""
Created on Fri Nov 13 19:08:56 2020
...
param_in = [[0.16,3]]#,[0.16,3]]

for p_ee, tau_s_di_ in param_in:
    for scale_ee_1 in [1]:#np.linspace(0.7,1.1,10):
        for cn_f in [1,2]:
            for ie_ratio_ in np.linspace(3.175, 3.575, 5):#np.linspace(2.0, 3.38, 10):
                loop_num += 1
...
ijwd.hybrid = 0.4
ijwd.cn_scale_weight = cn_f
ijwd.cn_scale_wire = cn_f
ijwd.iter_num=5
...
syn_ee_1.effect = 0; syn_ee_1.tau_s_de = 5.0*ms
syn_ie_1.effect = 0; syn_ie_1.tau_s_di = tau_s_di_*ms
syn_ei_1.effect = 0; syn_ei_1.tau_s_de = 5.0*ms
...
net.run(simu_time1, profile=True) #,namespace={'tau_k': 80*ms}
#extnl_e.rates = bkg_rate2e + stim_rate
group_e_1.delta_gk[chg_adapt_neuron] = 2*nS
net.run(simu_time2, profile=True) #,namespace={'tau_k': 80*ms}
#extnl_e.rates = bkg_rate2e
...
datasave = {'datetime':now.strftime("%Y-%m-%d %H:%M:%S"),
...
##%%
#net.restore('state1')
# data_yg = mydata.mydata()
# data_yg.load(datasave)
# #%%
