...
# -*- coding: utf-8 -*-
"""
Created on Tue Oct 27 11:37:23 2020
...
for scale_ee_1 in [1.200]:#np.arange(1.4,1.71,0.1): #np.linspace(1.,1.2,5):
    for scale_ei_1 in [1]:#scale_ee_1*np.linspace(0.9,1.1,5):
        for scale_ie_1 in np.arange(0.9025,1.2025,0.02):#np.arange(1.4,1.71,0.1):#[1]:#scale_ee_1*np.linspace(0.9,1.1,5):#np.arange(0.9825,1.1225,0.02):#[1.0625]: #np.linspace(0.95,1.1,5):
            for scale_ii_1 in [1]:#scale_ie_1*np.linspace(0.9,1.1,5):#np.linspace(1.,1.,1):
                for w_extnl_ in [1.5]:
                    for delta_gk_ in [3,7,10]:#[14]:#np.linspace(7,15,9):
                        for tau_k_ in [80]:#np.arange(40,121,10): 
                            for new_delta_gk_ in [delta_gk_]:#[14]:#np.arange(0.15,0.31,0.05)*delta_gk_: #np.linspace(1,5,5):
                                for tau_s_di_ in [4.444]:#np.arange(0.7,1.31,0.1)*4.444:#np.arange(3,6.7,0.3):#np.linspace(4.444,4.444,1):
                                    for tau_s_de_ in np.linspace(3.,5.8,10):#np.linspace(5.8,5.8,1):
