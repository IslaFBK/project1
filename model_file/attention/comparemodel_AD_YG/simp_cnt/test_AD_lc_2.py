...
# -*- coding: utf-8 -*-
"""
Created on Sun Oct 25 16:05:40 2020
...
for scale_ee_1 in [1.200]:#np.arange(1,1.26,0.05):#[1.15]: #np.linspace(1.,1.2,5):
    for scale_ei_1 in [1]:#np.linspace(1,1,1):
        for scale_ie_1 in [1.0225]:#np.arange(0.9825,1.1225,0.02):#[1.0625]: #np.linspace(0.95,1.1,5):
...
ijwd.decay_p_ei = 10 * 0.82#scale_d_p # decay constant of e to i connection probability as distance increases
ijwd.decay_p_ie = 20 * 0.82#scale_d_p # decay constant of i to e connection probability as distance increases
ijwd.decay_p_ii = 20 * 0.82#scale_d_p # decay constant of i to i connection probability as distance increases

ijwd.mean_SynNumIn_ee = 200#200 320 * 6/8 * 1   ; # p = 0.08
ijwd.mean_SynNumIn_ei = 320#500 500 #500 * 1      #500 #* 8/5     ; # p = 0.125
ijwd.mean_SynNumIn_ie = 129#125 200 * 5/8     ; # p = 0.2
ijwd.mean_SynNumIn_ii = 221#250 250 * 1    # 250 * 8/5     ; # p = 0.25
...
scale_ee = 1.0#1.05; 
scale_ei = 1.3 #1.
scale_ie = 1.115  #1.115 1.13 0.95 1.07
scale_ii = 1.03  #1.00            0.89
...
#tau_d = 100*ms
group_e_1.tau_s_di = 4*ms
group_i_1.tau_s_di = 4*ms
...
if allrun:
    #group_e_1.delta_gk[chg_adapt_neuron] = 2*nS
...
ax = firing_rate_analysis.plot_traj(ax, data1.a1.ge.centre_mass.centre[:2000])
ax.set_xlim([-0.5,62.5])
ax.set_ylim([-0.5,62.5])

#%%
data1.a1.ge.get_MSD(start_time=1000, end_time=4000, n_neuron=63*63,window = 15, jump_interval=np.array([15]), fit_stableDist='pylevy')
...
ax1.set_title('spon')
coef, freq = fa.myfft(mua_adpt[5000:14000], 1000)
freq_max = 100
ind_len = freq[freq<freq_max].shape[0] 
...
#%%
dt = 1/10000;
end = int(4/dt); start = int(0/dt)
spon_rate = np.sum((data1.a1.ge.t < end) & (data1.a1.ge.t > start))/4/data1.a1.param.Ne
print(spon_rate)
