...
# -*- coding: utf-8 -*-
"""
Created on Sun Nov 29 22:06:03 2020
...
# -*- coding: utf-8 -*-
"""
Created on Sun Nov 15 22:12:35 2020
        ...
        for num_ie in [150]:#[260]:#np.linspace(246, 300, 6,dtype=int):#[221]:#np.linspace(156, 297, 10,dtype=int):
            for num_ii in [230]:#np.linspace(135, 170,6,dtype=int):#[129]:#np.linspace(93, 177, 10,dtype=int):
                for ie_r_e in [2.76*6.5/5.8]:# np.linspace(3.07,3.13,7):#np.arange(2.5,4.0,0.04):#[3.1]:#np.linspace(3.10,3.45,6):#np.linspace(1.4,1.6,10):#[1.5]: #np.linspace(1.,1.2,5):
                    for ie_r_e1 in np.arange(0.96,1.041,0.02):
                        for ie_r_e2 in np.arange(0.96,1.041,0.02):
                                                            ...
                                                            for tau_s_di_ in np.linspace(4.4,4.4,1):
                                                                for tau_s_de_ in np.linspace(5.,5.,1):
                                                                    for tau_s_r_ in [1]:#np.linspace(1,1,1):
                                                                        for scale_w_12 in 0.18*np.arange(0.8,1.21,0.05):
                                                                            for scale_w_21 in 0.22*np.arange(0.8,1.21,0.05):
...
ijwd2.w_ie_mean = find_w_i(w_ee_, num_ee, num_ie, ie_r_e)#w_ie_ #find_w_i(w_ee_, num_ee, num_ie, ie_r_e)
...
ijwd_inter.w_e1_e2_mean *= 5*scale_w_12; ijwd_inter.w_e1_i2_mean *= 5*scale_w_12
ijwd_inter.w_e2_e1_mean *= 5*scale_w_21; ijwd_inter.w_e2_i1_mean *= 5*scale_w_21
...
chg_adapt_loca = [0, 0]
chg_adapt_range = 6 
chg_adapt_neuron = cn.findnearbyneuron.findnearbyneuron(ijwd2.e_lattice, chg_adapt_loca, chg_adapt_range, ijwd1.width)
...
stim_rate = psti.input_spkrate(maxrate = [800,800], sig=[6,6], position=[[0, 0]])*Hz
...
simu_time_tot = 18000*ms
#group_input.active = False
extnl_e1.rates = 0*Hz
net.run(simu_time1, profile=False) #,namespace={'tau_k': 80*ms}
#extnl_e1.rates = bkg_rate2e + stim_rate
extnl_e1.rates = stim_rate
