...
# -*- coding: utf-8 -*-
"""
Created on Wed Nov 25 13:13:44 2020
...
spon_rate = np.sum((data.a1.ge.t < end) & (data.a1.ge.t > start))/3/data.a1.param.Ne
...
# if analy_type == 'ie_hz':
#     #data_anly.invalid_pei = data.a1.param.p_peak_ei >= 1    
#     title = "erie%.3f_ee%.2f_ei%.2f_\non%.2f_sz%.1f_alpha%.2f_hz%.2f"%(data.a1.param.ie_r_e,data.a1.param.w_ee_mean,data.a1.param.w_ei_mean,\
#                                                       data_anly.pattern_on_ratio, data_anly.patterns_size_mean,\
#                                                       data.a1.ge.MSD.stableDist_param[0,0],spon_rate)    
if analy_type == 'ie_hz':
    #data_anly.invalid_pei = data.a1.param.p_peak_ei >= 1    
    title = "erie%.3f_ie%.2f_ei%.2f_\non%.2f_sz%.1f_alpha%.2f_hz%.2f"%(data.a1.param.ie_r_e,data.a1.param.w_ie_mean,data.a1.param.w_ei_mean,\
                                                      data_anly.pattern_on_ratio, data_anly.patterns_size_mean,\
                                                      data.a1.ge.MSD.stableDist_param[0,0],spon_rate)    
if analy_type == 'fft':
    #data_anly.invalid_pei = data.a1.param.p_peak_ei >= 1    
...
if analy_type == 'cfc':
    #data_anly.invalid_pei = data.a1.param.p_peak_ei >= 1    
    title = "erie%.3f_hz%.2f"%(data.a1.param.ie_r_e,\
                                                      spon_rate)    

if analy_type == 'cwt':
    #data_anly.invalid_pei = data.a1.param.p_peak_ei >= 1    
    title = "erie%.3f_wext%.1f_hz%.2f"%(data.a1.param.ie_ratio, data.a1.param.w_extnl,\
                                                      spon_rate)    
...
"""
#%%
lattice_ext = cn.coordination.makelattice(int(np.sqrt(data.a1.param.Ne).round()),data.a1.param.width,[0,0])
...
mua_neuron_1 = cn.findnearbyneuron.findnearbyneuron(lattice_ext, mua_loca_n, mua_range, data.a1.param.width)
mua_loca_n = [31.5,31.5]; mua_loca.append(mua_loca_n)
mua_range = 5#6 * scale_d_p
mua_neuron_2 = cn.findnearbyneuron.findnearbyneuron(lattice_ext, mua_loca_n, mua_range, data.a1.param.width)


start_time = 4e3; end_time = 19e3
data.a1.ge.get_spike_rate(start_time=start_time, end_time=end_time, sample_interval=1, n_neuron = data.a1.param.Ne, window = 10)
mua_1 = data.a1.ge.spk_rate.spk_rate.reshape(data.a1.param.Ne,-1)[mua_neuron_1]
mua_1 = mua_1.mean(0)/0.01
mua_2 = data.a1.ge.spk_rate.spk_rate.reshape(data.a1.param.Ne,-1)[mua_neuron_2]
mua_2 = mua_2.mean(0)/0.01

mua = np.vstack((mua_1,mua_2)) 
#%%
Fs = 1000;
timeDim = 0;
...
findcfc.optionSur = 2
#%%
MI_raw_mat, MI_surr_mat = findcfc.find_cfc_from_rawsig(mua)
...
for i in range(len(MI_raw_mat)):
    ...
    plt.suptitle('erie: %.3f, location: %s'%(data.a1.param.ie_ratio, mua_loca[i]))
...
data_anly.cfc = mydata.mydata()
data_anly.cfc.MI_raw_mat = MI_raw_mat
data_anly.cfc.MI_surr_mat = MI_surr_mat
data_anly.cfc.phaseBand = phaseBand_in
data_anly.cfc.ampBand = ampBand_in
#%%
'''cwt'''
...
savetitle = title.replace('\n','')
cwtfile = savetitle+'_cwt_%d'%loop_num+'.png'
fig.savefig(cwtfile)
cwtfile = savetitle+'_cwt_%d'%loop_num+'.svg'
fig.savefig(cwtfile)
