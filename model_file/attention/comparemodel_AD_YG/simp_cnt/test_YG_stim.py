...
# -*- coding: utf-8 -*-
"""
Created on Wed Nov 25 12:59:48 2020
...
param_in = [[0.08,6.5]]
repeat = 5
for p_ee, tau_s_di_ in param_in:
    for scale_ee_1 in np.linspace(1,1,1):
        for w_extnl_ in [1.5, 2]:
            for delta_gk_ in [3]:
                for tau_s_de_ in [5.8]:
                    for ie_ratio_ in [3.375*0.98]*repeat:#(np.arange(0.7,1.56,0.05)-0.02):#np.linspace(2.4, 4.5, 20):
                        loop_num += 1
...
#%%
LFP_elec = np.meshgrid(np.linspace(-31.5, 31.5-(63/4), 4), np.linspace(31.5-(63/4), -31.5, 4))
LFP_elec = np.concatenate((LFP_elec[0].reshape(-1,1), LFP_elec[1].reshape(-1,1)),1)
#LFP_elec[5,:] = np.array([-1,1]) 
#%%
def LFP_w(lattice_ext, LFP_elec):
    i = np.array([], int);
    j = np.array([], int);
    w = np.array([], float);
    sigma = 8.
    for k in range(len(LFP_elec)):
        dist = cn.coordination.lattice_dist(lattice_ext,62,LFP_elec[k])
...
#        ii = np.where(ww >= 0.01)[0]
    ...
    return i, j, w
#%%
lfpe_i, lfpe_j, lfpe_w =  LFP_w(ijwd.lattice_ext, LFP_elec)

param_new = {'LFP_loca':LFP_elec, 'LFP_sigma':8}
param = {**param, **param_new}
#%%
#plt.figure()
#plt.scatter(ijwd.lattice_ext[lfpe_i[lfpe_j==0]][:,0],ijwd.lattice_ext[lfpe_i[lfpe_j==0]][:,1], s=10*lfpe_w[lfpe_j==0])  
##%%
...
neuronmodel_e = model_neu_syn_YG.neuron_e_YG

neuronmodel_i = model_neu_syn_YG.neuron_i_YG

synapse_e = model_neu_syn_YG.synapse_e_YG
synapse_i = model_neu_syn_YG.synapse_i_YG
#%%
...
tau_x_re = 1*ms
syn_extnl_e = Synapses(extnl_e, group_e_1, method='euler', on_pre='x_extnl_post += 1/tau_x_re')
#syn_extnl_i = Synapses(extnl_i, group_i_1, method='euler', on_pre='x_extnl_post += 1/tau_x_re')
...
syn_ee_1.effect = 0; syn_ee_1.tau_s_de = tau_s_de_*ms; syn_ee_1.tau_s_re = 1*ms
syn_ie_1.effect = 0; syn_ie_1.tau_s_di = tau_s_di_*ms; syn_ie_1.tau_s_ri = 1*ms
syn_ei_1.effect = 0; syn_ei_1.tau_s_de = tau_s_de_*ms; syn_ei_1.tau_s_re = 1*ms
syn_ii_1.effect = 0; syn_ii_1.tau_s_di = tau_s_di_*ms; syn_ii_1.tau_s_ri = 1*ms
...
group_e_1.w_extnl = w_extnl_*nS
group_i_1.w_extnl = w_extnl_*nS
group_e_1.tau_s_de_extnl = tau_s_de_*ms
group_i_1.tau_s_de_extnl = tau_s_de_*ms
...
group_e_1.g_E_inter = 0*nS
group_i_1.g_E_inter = 0*nS
...
lfp_moni = StateMonitor(group_LFP_record, ('lfp1', 'lfp2'), record = True)
#%%
net = Network(collect())
net.store('state1')
#%%
...
extnl_e.rates = stim_rate #bkg_rate2e + stim_rate
...
data = {'datetime':now.strftime("%Y-%m-%d %H:%M:%S"), 'dt':0.1,
        'a1':{'param':param,
              'LFP':{'lfp1':lfp_moni.lfp1/nA, 'lfp2': lfp_moni.lfp2/nA},
