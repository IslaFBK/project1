...
# -*- coding: utf-8 -*-
"""
Created on Sun Nov  8 22:11:48 2020
...
ijwd.decay_p_ei = 9#*1.125#scale_d_p # decay constant of e to i connection probability as distance increases
...
ijwd.mean_SynNumIn_ee = 265#200 320 * 6/8 * 1   ; # p = 0.08                          200
ijwd.mean_SynNumIn_ei = 395#500 500 #500 * 1      #500 #* 8/5     ; # p = 0.125       320  
ijwd.mean_SynNumIn_ie = 170#125 200 * 5/8     ; # p = 0.2                             129
ijwd.mean_SynNumIn_ii = 285#250 250 * 1    # 250 * 8/5     ; # p = 0.25               221  
...
scale_ee = 1.0#1.05;                                                                
scale_ei = 1.05 #1.
scale_ie = 1.09 #1.115 1.13 0.95 1.07                           #0.87
scale_ii = 0.89   #1.00            0.89                          #0.83
...
data1.a1.ge.get_spike_rate(start_time=0, end_time=14000,\
                           sample_interval = 1, n_neuron = ijwd.Ne, window = 5, dt = 0.1, reshape_indiv_rate = True)

mua_adpt = data1.a1.ge.spk_rate.spk_rate.reshape(ijwd.Ne,-1)[chg_adapt_neuron].mean(0)/0.005
#%%
plt.figure()
...
coef, freq = fa.myfft(mua_adpt[4000:10000], 1000)
freq_max = 20
...
#%%
dt = 1/10000;
end = int(2/dt); start = int(1/dt)
spon_rate = np.sum((data1.a1.ge.t < end) & (data1.a1.ge.t > start))/1/data1.a1.param.Ne
print(spon_rate)
spon_rate = np.sum((data1.a1.gi.t < end) & (data1.a1.gi.t > start))/1/data1.a1.param.Ni
print(spon_rate)
