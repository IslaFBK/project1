...
# -*- coding: utf-8 -*-
"""
Created on Sat Oct 31 16:30:01 2020
...
data.a1.ge.get_MSD(start_time=1000, end_time=5000, n_neuron=data.a1.param.Ne, window = 15, jump_interval=np.array([15]), fit_stableDist='pylevy')
...
title = "ee%.2f_ei%.2f_ie%.2f_ii%.2f_dsi%.2f_dse%.2f_alpha%.2f_hz%.2f"%(data.a1.param.scale_ee_1, data.a1.param.scale_ei_1,\
...
fftfile = savetitle+'_pf%.1f_fft_%d'%(peakF,loop_num)+'.png'
fig.savefig(fftfile)

#plt.plot(freq[1:], np.abs(coef[1:]))
#%%
'''move good results'''
if peakF > 2.8 and peakF < 8 and spon_rate > 3 and \
    spon_rate < 13 and data.a1.ge.MSD.stableDist_param[0,0]>1.3 \
        and data.a1.ge.MSD.stableDist_param[0,0] < 1.7:
