...
# -*- coding: utf-8 -*-
"""
Created on Fri Nov 13 22:31:24 2020
    ...
    for scale_d_p_ei in [1]:#np.linspace(0.85,1.15,13):#[1]
        for scale_d_p_ii in np.linspace(1,1,1):
            for scale_d_p_ie in np.linspace(1,1,1):#np.linspace(0.85,1.15,13):
                for num_ee in np.linspace(220,260,5):#[260]:#np.linspace(125, 297, 10,dtype=int): 
                    for num_ei in [400]:#np.linspace(204, 466, 10, dtype=int):#[320]:               
                        for num_ie in [170]:#[260]:#np.linspace(246, 300, 6,dtype=int):#[221]:#np.linspace(156, 297, 10,dtype=int):
                            for num_ii in np.linspace(240,270,4):#[260]:#np.linspace(135, 170,6,dtype=int):#[129]:#np.linspace(93, 177, 10,dtype=int):
                                for ie_r_e in np.arange(2.8,3.4,0.03):#[3.1]:#np.linspace(3.10,3.45,6):#np.linspace(1.4,1.6,10):#[1.5]: #np.linspace(1.,1.2,5):
                                    for w_ee_ in [4]:#np.linspace(17,23,7):#np.linspace(1.077,1.277,10):#[1.177]:#np.linspace(1,1,1):
                                        for ie_r_i in (ie_r_e/1.2)*np.arange(0.9,1.12,0.03):#np.arange(2.4,2.8,0.):#[2.786]:#np.linspace(2.5,3.0,15):#np.linspace(1.156-0.08,1.156+0.08,15):#np.linspace(1.156-0.175,1.156+0.175,31):#np.linspace(1.157,1.357,10):#[1.257]: #np.linspace(0.95,1.1,5):
                                            for w_ii_ in [25]:#np.linspace(20,27,8):#np.arange(1.4,1.81,0.1)]]:#np.linspace(1.,1.,1):
                                                for w_extnl_ in [1.5]:
                                                    for delta_gk_ in [10]:#np.linspace(7,15,9)]]:
                                                        for new_delta_gk_ in [10/5]:#np.arange(0.15,0.31,0.05)*delta_gk_:#np.linspace(1,5,5):
...
ijwd1.decay_p_ei = 9.5 #* scale_d_p_ei# scale_d_p # decay constant of e to i connection probability as distance increases
