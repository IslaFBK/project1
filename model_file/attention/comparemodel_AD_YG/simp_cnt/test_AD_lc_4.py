...
# -*- coding: utf-8 -*-
"""
Created on Mon Nov  2 14:01:14 2020
...
for scale_ee_1 in [1.200]:#np.arange(1,1.26,0.05):#[1.15]: #np.linspace(1.,1.2,5):
    for scale_ei_1 in [1.27]:#np.linspace(1,1,1):
        for scale_ie_1 in [1.116]:#np.arange(0.9825,1.1225,0.02):#[1.0625]: #np.linspace(0.95,1.1,5):
...
ijwd.decay_p_ee = 8  * 0.8#scale_d_p # decay constant of e to e connection probability as distance increases
ijwd.decay_p_ei = 10*0.82#*1.125#scale_d_p # decay constant of e to i connection probability as distance increases
ijwd.decay_p_ie = 20*0.86 *1.15#scale_d_p # decay constant of i to e connection probability as distance increases
ijwd.decay_p_ii = 20*0.86 *1.15#scale_d_p # decay constant of i to i connection probability as distance increases

ijwd.mean_SynNumIn_ee = 200#200 320 * 6/8 * 1   ; # p = 0.08
ijwd.mean_SynNumIn_ei = 320#500 500 #500 * 1      #500 #* 8/5     ; # p = 0.125
ijwd.mean_SynNumIn_ie = 148#125 200 * 5/8     ; # p = 0.2
ijwd.mean_SynNumIn_ii = 254#250 250 * 1    # 250 * 8/5     ; # p = 0.25
...
scale_ee = 1.0#1.05; 
scale_ei = 1.0 #1.
scale_ie = 1.0 #1.115 1.13 0.95 1.07
scale_ii = 1.0   #1.00            0.89
...
#%%
centre_ind, centre, jump_size, jump_dist, pattern, pattern_size = firing_rate_analysis.get_centre_mass2(data1.a1.ge.spk_rate.spk_rate, 1, 1, 15, detect_pattern=True)
#%%
ani = firing_rate_analysis.show_pattern(data1.a1.ge.spk_rate.spk_rate, None, \
                                        ...
                                        show_pattern_size=True, pattern_ctr=data1.a1.ge.centre_mass.centre_ind, pattern_size=pattern_size)
#%%
...
coef, freq = fa.myfft(mua_adpt[1000:4000], 1000)
freq_max = 20
...
ax1.set_title('spon')
coef, freq = fa.myfft(mua_adpt[6000:14000], 1000)
freq_max = 20
ind_len = freq[freq<freq_max].shape[0] 
...
spon_rate = np.sum((data1.a1.ge.t < end) & (data1.a1.ge.t > start))/6/data1.a1.param.Ne
print(spon_rate)
