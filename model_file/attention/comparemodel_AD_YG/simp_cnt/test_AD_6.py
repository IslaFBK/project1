...
# -*- coding: utf-8 -*-
"""
Created on Wed Oct 21 11:11:28 2020
...
for scale_ee_1 in [1.15]: #np.linspace(1.,1.2,5):
    for scale_ei_1 in np.arange(1.4,1.81,0.1):#np.linspace(1,1,1):
        for scale_ie_1 in [1.0625]: #np.linspace(0.95,1.1,5):
            for scale_ii_1 in np.arange(1.4,1.81,0.1):#np.linspace(1.,1.,1):
                for w_extnl_ in [1.5]:
                    for delta_gk_ in np.linspace(7,15,9):
                        for new_delta_gk_ in np.arange(0.15,0.31,0.05)*delta_gk_:#np.linspace(1,5,5):
