# -*- coding: utf-8 -*-
"""
Created on Sun Oct 25 22:43:08 2020

@author: nishe
"""

'''
increase excitatory coupling strength, i.e. E-I and E-E
...
for scale_ee_1 in np.arange(1.4,1.71,0.1): #np.linspace(1.,1.2,5):
    for scale_ei_1 in scale_ee_1*np.linspace(0.9,1.1,5):
        for scale_ie_1 in np.arange(1.4,1.71,0.1):#[1]:#scale_ee_1*np.linspace(0.9,1.1,5):#np.arange(0.9825,1.1225,0.02):#[1.0625]: #np.linspace(0.95,1.1,5):
            for scale_ii_1 in scale_ie_1*np.linspace(0.9,1.1,5):#np.linspace(1.,1.,1):
                for w_extnl_ in [1.5]:
                    for delta_gk_ in [14]:#np.linspace(7,15,9):
                        for tau_k_ in [90]:#np.arange(40,121,10): 
                            for new_delta_gk_ in [3.5]:#np.arange(0.15,0.31,0.05)*delta_gk_: #np.linspace(1,5,5):
                                for tau_s_di_ in np.linspace(4.444,4.444,1):
                                    for tau_s_de_ in np.linspace(5.8,5.8,1):
...
ijwd.mean_SynNumIn_ee = 320     ; # p = 0.08
...
ijwd.w_ee *= scale_ee_1*1.15#tau_s_de_
ijwd.w_ei *= scale_ei_1 #tau_s_de_ #5*nS
ijwd.w_ie *= scale_ie_1*1.042#tau_s_di_#25*nS
...
param_new = {'delta_gk':delta_gk_,
         ...
         'scale_ee_1': scale_ee_1*1.15,
         'scale_ei_1': scale_ei_1,
         'scale_ie_1': scale_ie_1*1.042,
