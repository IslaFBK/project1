...
# -*- coding: utf-8 -*-
"""
Created on Mon Nov 16 16:59:50 2020
...
for scale_d_p_ee in [1]:#np.linspace(0.85,1.15,13):
    for scale_d_p_ei in [1]:#np.linspace(0.85,1.15,13):#[1]
        for scale_d_p_ii in np.linspace(0.8,0.95,8):
            for scale_d_p_ie in [scale_d_p_ii]:#np.linspace(1,1,1):#np.linspace(0.85,1.15,13):
                for num_ee in [230]:#np.linspace(220,260,5,dtype=int):#[260]:#np.linspace(125, 297, 10,dtype=int): 
                    for num_ei in [400]:#np.linspace(204, 466, 10, dtype=int):#[320]:               
                        for num_ie in [150]:#np.linspace(140,170,4):#[170]:#[260]:#np.linspace(246, 300, 6,dtype=int):#[221]:#np.linspace(156, 297, 10,dtype=int):
                            for num_ii in np.linspace(230,230,1,dtype=int):#[260]:#np.linspace(135, 170,6,dtype=int):#[129]:#np.linspace(93, 177, 10,dtype=int):
                                        ...
                                        for ie_r_e in np.arange(2.8,3.4,0.03):#[3.1]:#np.linspace(3.10,3.45,6):#np.linspace(1.4,1.6,10):#[1.5]: #np.linspace(1.,1.2,5):
                                            for w_ei_ in [5]:#np.linspace(5,7,5):
                                                for w_ii_ in [None]:#np.linspace(20,27,8):#np.arange(1.4,1.81,0.1)]]:#np.linspace(1.,1.,1):
                                                    for ie_r_i in (ie_r_e/1.2)*np.arange(0.84,1.18,0.03):#np.arange(2.4,2.8,0.):#[2.786]:#np.linspace(2.5,3.0,15):#np.linspace(1.156-0.08,1.156+0.08,15):#np.linspace(1.156-0.175,1.156+0.175,31):#np.linspace(1.157,1.357,10):#[1.257]: #np.linspace(0.95,1.1,5):
                                                        for w_extnl_ in [1.5]:
