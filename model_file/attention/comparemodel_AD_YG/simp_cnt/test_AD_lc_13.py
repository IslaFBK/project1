...
# -*- coding: utf-8 -*-
"""
Created on Thu Nov 19 15:35:36 2020
...
prefs.codegen.max_cache_dir_size = 300
#%%
    ...
    for scale_d_p_ei in [1]:#np.linspace(0.85,1.15,13):#[1]
        for scale_d_p_ii in np.linspace(1,1,1):
            for scale_d_p_ie in np.linspace(1,1,1):#np.linspace(0.85,1.15,13):
                for num_ee in [240]:#np.linspace(220,260,5,dtype=int):#[260]:#np.linspace(125, 297, 10,dtype=int): 
                        ...
                        for num_ie in [150]:#np.linspace(140,170,4):#[170]:#[260]:#np.linspace(246, 300, 6,dtype=int):#[221]:#np.linspace(156, 297, 10,dtype=int):
                            for num_ii in [230]:#np.linspace(240,240,1,dtype=int):#[260]:#np.linspace(135, 170,6,dtype=int):#[129]:#np.linspace(93, 177, 10,dtype=int):
                                for w_ee_ in [4]:#np.linspace(17,23,7):#np.linspace(1.077,1.277,10):#[1.177]:#np.linspace(1,1,1):
                                    for w_ie in [None]:
                                        for ie_r_e in [2.760]:#np.arange(2.8,3.4,0.03):#[3.1]:#np.linspace(3.10,3.45,6):#np.linspace(1.4,1.6,10):#[1.5]: #np.linspace(1.,1.2,5):
                                            for w_ei_ in [5]:
                                                for w_ii_ in [None]:#np.linspace(20,27,8):#np.arange(1.4,1.81,0.1)]]:#np.linspace(1.,1.,1):
                                                    for ie_r_i in [2.450]:#(ie_r_e/1.2)*np.arange(0.9,1.12,0.03):#np.arange(2.4,2.8,0.):#[2.786]:#np.linspace(2.5,3.0,15):#np.linspace(1.156-0.08,1.156+0.08,15):#np.linspace(1.156-0.175,1.156+0.175,31):#np.linspace(1.157,1.357,10):#[1.257]: #np.linspace(0.95,1.1,5):
                                                        for w_extnl_ in [1.5]:
...
multimeter_E = StateMonitor(group_e_1, ('I_inh','I_exi'), record = [0,2000])
multimeter_I = StateMonitor(group_i_1, ('I_inh','I_exi'), record = [0,500])
...
scale_ee = 1#2.#1.05;                                                                
scale_ei = 1.0#4/5 #1.
scale_ie = 1.0#2.0   #1.115 1.13 0.95 1.07                           #0.87
...
#scale_tau_k = 6/8
group_e_1.delta_gk = 16*nS #/ scale_tau_k
group_e_1.tau_k = 60*ms #* scale_tau_k

group_e_1.C = 0.25*nF #0.5*nF
group_i_1.C = 0.25*nF #0.5*nF
...
if all_run:
    group_e_1.delta_gk[chg_adapt_neuron] = 3*nS
    group_e_1.tau_k[chg_adapt_neuron] = 30*ms
    #group_e_1.delta_gk[:] = new_delta_gk_*nS
...
print(data1.a1.ge.centre_mass.pattern.sum()/len(data1.a1.ge.centre_mass.pattern))
...
data1.a1.ge.get_spike_rate(start_time=0, end_time=24000,\
                           sample_interval = 1, n_neuron = ijwd1.Ne, window = 10, dt = 0.1, reshape_indiv_rate = True)

mua_adpt = data1.a1.ge.spk_rate.spk_rate.reshape(ijwd1.Ne,-1)[chg_adapt_neuron].mean(0)/0.01
data1.a1.ge.get_centre_mass(detect_pattern=False)
#%%
plt.figure()
plt.plot(data1.a1.ge.t*0.1, data1.a1.ge.i,'.')
#%%
data1.a1.ge.get_sparse_spk_matrix([3969,200000])
#data1.a1.ge.spk_matrix[chg_adapt_neuron].i
#%%
plt.figure()
plt.plot(data1.a1.ge.spk_matrix[chg_adapt_neuron].nonzero()[1], data1.a1.ge.spk_matrix[chg_adapt_neuron].nonzero()[0],'.')

#%%
plt.figure()
plt.plot(mua_adpt[4000:20000])
#%%
def find_peakF(coef, freq):
    ...
    return peakF
#%%
fft_data = data1.a1.ge.centre_mass.centre[:,0]
#%%
fft_data = mua_adpt

fig, [ax1,ax2] = plt.subplots(1,2,figsize=[8,4])
coef, freq = fa.myfft(fft_data[10000:15000], 1000)
freq_max = 20
...
ax1.set_title('spon')
coef, freq = fa.myfft(fft_data[6000:16000], 1000)
freq_max = 20
ind_len = freq[freq<freq_max].shape[0] 
...
#%%
plt.figure()
plt.plot(multimeter_E.t/ms, multimeter_E.I_exi[0]/nA, label='excitatory current')
plt.plot(multimeter_E.t/ms, multimeter_E.I_inh[0]/nA, label='inhibitory current')
plt.legend()
plt.xlabel('ms')
plt.ylabel('nA')
plt.title('excitatory and inhibitory synaptic current \nreceived by a neuron when the network is at working region')
