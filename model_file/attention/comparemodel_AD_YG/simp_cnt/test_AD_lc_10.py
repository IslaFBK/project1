...
# -*- coding: utf-8 -*-
"""
Created on Sat Nov 14 12:41:10 2020
                ...
                for w_extnl_ in [1.5]:
                    for delta_gk_ in [12]:#np.linspace(7,14,8):
                        for new_delta_gk_ in [2.5]:#np.arange(0.15,0.31,0.05)*delta_gk_:#np.linspace(1,5,5):
                            for tau_s_di_ in [4.4]:#np.arange(0.7,1.31,0.1)*4.444:#np.linspace(4.444,4.444,1):
                                for tau_s_de_ in [5.]:#np.arange(0.7,1.11,0.1)*5.8:#np.linspace(5.8,5.8,1):
...
group_i_1.g_l = 16.7*nS #25*nS
group_i_1.C = 0.25*nF
group_i_1.t_ref = 3*ms
...
scale_ee = 1.#1.05;                                                                
scale_ei = 1.0 #1.
scale_ie = 1.0 * 1.01 #1.115 1.13 0.95 1.07                           #0.87
scale_ii = 1. * 1  #1.00            0.89                          #0.83
...
group_e_1.I_extnl_crt = 0.60*nA #0.51*nA
group_i_1.I_extnl_crt = 0.60*nA #0.60*nA
...
if allrun:
    group_e_1.delta_gk[chg_adapt_neuron] = 2.0*nS
...
coef, freq = fa.myfft(mua_adpt[4000:16000], 1000)
freq_max = 20
