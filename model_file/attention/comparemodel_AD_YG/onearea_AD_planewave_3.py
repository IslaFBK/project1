...
# -*- coding: utf-8 -*-
"""
Created on Thu Aug  6 00:45:35 2020
...
try tuning common-neighbour factor
'''
...
for p_ee_ in [0.16]: 
    for cn_scale_wire_ in np.linspace(1.5,2.5,10):
                ...
                for t_ref in np.linspace(4,4,1)*ms:
                    for scale_ee_1 in [1]: #np.linspace(0.7,1.1,10):
                        for tau_p_ei_ in np.linspace(10,10,1):
                            for w_ei_ in np.linspace(5,5,1)*10**-3:
                                for w_ii_ in np.linspace(25,25,1)*10**-3:
                                    for ie_ratio_ in np.linspace(2.5, 3.2, 8): #np.linspace(2.4, 2.7, 5)
                                        loop_num += 1
