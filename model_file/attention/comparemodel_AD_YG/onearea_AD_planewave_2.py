        ...
        for delta_gk_ in np.linspace(10, 10, 1):
            for t_ref in np.linspace(4,4,1)*ms:
                for scale_ee_1 in [1]: #np.linspace(0.7,1.1,10):
                    for tau_p_ei_ in np.linspace(9,10,2):
                        for w_ei_ in np.linspace(4,5,5)*10**-3:
                            for w_ii_ in np.linspace(25,28,5)*10**-3:
                                for ie_ratio_ in np.linspace(2.6, 3.5, 10): #np.linspace(2.4, 2.7, 5)
                                    loop_num += 1
