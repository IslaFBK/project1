...
for p_ee_ in [0.16]: 
    for tau_s_di_ in np.linspace(3, 6, 5):
        for delta_gk_ in np.linspace(11, 20, 10):
            for t_ref in np.linspace(4,6,5)*ms:
                for scale_ee_1 in [1]: #np.linspace(0.7,1.1,10):
                    for ie_ratio_ in np.linspace(2.5, 3., 5):
                        loop_num += 1
...
param = {'delta_gk':delta_gk_,
         ...
         'tau_s_di':tau_s_di_,
         'tau_s_de':5.0, 'ie_ratio':ie_ratio,
