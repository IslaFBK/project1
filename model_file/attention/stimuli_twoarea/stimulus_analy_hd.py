...
# -*- coding: utf-8 -*-
"""
Created on Fri Mar 13 21:41:24 2020
...
timepath = '/headnode1/shni2598/brian2/data/twoarea_stimuli/timev%d%s.mat'#%(area,file_num)
indexpath = '/headnode1/shni2598/brian2/data/twoarea_stimuli/indexv%d%s.mat'##%(area,file_num)

spkv1 = psa.importmatdata(timepath, indexpath, file_num=file_num, area=1)
spkv2 = psa.importmatdata(timepath, indexpath, file_num=file_num, area=2)

spkrate1, centre_ind1, jump_size1, jump_dist1 = psa.get_rate_centre_jumpdist(spkv1, starttime=1000*ms, endtime=6000*ms, \
      binforrate=10*ms, sample_interval=1*ms, slide_interval = 1, jump_interval = 15, dt=0.1*ms, show_trajectory=False)
spkrate2, centre_ind2, jump_size2, jump_dist2 = psa.get_rate_centre_jumpdist(spkv2, starttime=1000*ms, endtime=6000*ms, \
      binforrate=10*ms, sample_interval=1*ms, slide_interval = 1, jump_interval = 15, dt=0.1*ms, show_trajectory=False)
...
ani.save('%.1f_%.1f.mp4'%(scale_e_12[i],scale_e_21[j]))
