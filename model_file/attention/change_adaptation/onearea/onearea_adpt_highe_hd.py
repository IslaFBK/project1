# -*- coding: utf-8 -*-
"""
Created on Thu May  7 21:31:32 2020
...
for ie_ratio in 3.375*np.arange(0.9, 1.21, 0.03):
    for tau_s_di in np.arange(3., 6.1, 0.5): # ms
        for new_delta_gk in np.arange(0, 8.1, 1): # nS
            for delta_gk in np.arange(6, 14.1, 1):
                loop_num += 1
                if loop_num == sys_argv:
                    print(loop_num)
                    break
...
#%
spk_tstep = np.round(spk_e1.t/(0.1*ms)).astype(int)
param = {'delta_gk':delta_gk,'new_delta_gk':new_delta_gk,'tau_s_di':tau_s_di/ms,'ie_ratio':ie_ratio}
