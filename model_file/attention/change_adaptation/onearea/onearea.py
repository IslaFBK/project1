# -*- coding: utf-8 -*-
"""
Created on Wed May  6 12:07:16 2020

@author: nishe
"""
'''
Tune the network to produce slow moving speed pattern with its distribution of pattern jump distance is heavy tail.
...
sys_argv = int(sys.argv[1])
loop_num = -1
for tau_s_di in [3., 6.]: # ms
    for new_delta_gk in np.arange(0, 8.1, 1): # nS
        loop_num += 1
        if loop_num == sys_argv:
            print(loop_num)
            break
...
ijwd.ie_ratio = 3.375 #3.375 #* scale_ie_ratio[i]
ijwd.mean_J_ee = 4*10**-3 #* 1.4 # usiemens
ijwd.sigma_J_ee = 1.9*10**-3 #* 1.4# usiemens
...
#%%
spkrate1, centre_ind1, jump_size1, jump_dist1 = psa.get_rate_centre_jumpdist(spk_e1, starttime=0*ms, endtime=6000*ms, binforrate=10*ms, sample_interval=1*ms, slide_interval = 1, jump_interval = 15, dt=0.1*ms, show_trajectory=False)
