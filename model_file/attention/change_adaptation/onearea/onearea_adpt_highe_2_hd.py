...
sys_argv = int(sys.argv[1])
loop_num = -1
for chg_adapt_range in np.arange(6,9):
    for ie_ratio in 3.375*np.arange(0.9, 1.21, 0.05):
        for tau_s_de in np.arange(4., 6.1, 0.5):
            for tau_s_di in np.arange(3., 6.1, 0.5): # ms
                for new_delta_gk in np.arange(0, 3.1, 1): # nS
                    for delta_gk in np.arange(8, 14.1, 1):
                        loop_num += 1
                        if loop_num == sys_argv:
                            print(loop_num)
                            break
...
param = {'delta_gk':delta_gk,'new_delta_gk':new_delta_gk,'tau_s_di':tau_s_di/ms,\
         'tau_s_de':tau_s_de/ms, 'ie_ratio':ie_ratio, 'adpt_range':chg_adapt_range}
