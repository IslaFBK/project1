# -*- coding: utf-8 -*-
"""
Created on Tue May 26 12:32:36 2020
...
#%%
scale_ee = 1.4; scale_ei = 1.4*1.03; scale_ii =1.4; #scale_ie_ratio = 1

sys_arg = int(sys.argv[1]); 
loop_num = -1
for scale_ie_ratio in np.arange(0.95, 1.1, 0.03):
    for delta_gk in np.arange(8, 13, 1):
        for new_delta_gk in np.arange(0, 2.1, 1):
            for tau_s_de in np.arange(4, 6.1, 0.5):
                for tau_s_di in np.arange(2.5, 5.1, 0.5):
                    for chg_adapt_range in np.arange(6, 8.1, 1):
                        loop_num += 1
                        if loop_num == sys_arg:
                            print(loop_num); break
                    else:continue
                    break
                else:continue
                break
...
ijwd.w_ei = 5*10**(-3) * scale_ei#* scale_ei * 1.05#usiemens
...
w : siemens'''
#%%
tau_s_de = tau_s_de*ms #5
tau_s_di = tau_s_di*ms #3 # Yuxi's parameter 6
tau_s_re = 1*ms
tau_s_ri = 1*ms
#%%
...
group_e.delta_gk[chg_adapt_neuron] = new_delta_gk*nS; group_e.tau_k[chg_adapt_neuron] = 30*ms
network.run(7000*ms,report='text')

#%%
spk_tstep = np.round(spk_e1.t/(0.1*ms)).astype(int)
param = {'ie_ratio': ie_ratio,'delta_gk':delta_gk, 'new_delta_gk':new_delta_gk, 'tau_s_de':tau_s_de/ms, 'tau_s_di':tau_s_di/ms, 'chg_adapt_range':chg_adapt_range,'loop_num':loop_num}
