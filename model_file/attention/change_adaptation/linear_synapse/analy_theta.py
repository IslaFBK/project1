...
with open(fileadr+'data%s'%(loop_num), 'rb') as file:
    spke1 = load_data_dict.data_onegroup(pickle.load(file))

spke1.t = spke1.t*0.1*ms
#%%
chg_adapt_loca = [0, 0]
chg_adapt_range = spke1.param['adpt_range']
...
#%%
savename = (spke1.param['adpt_range'], spke1.param['new_tau_k'],spke1.param['new_delta_gk'], \
...
#matcmd = "saveas(gcf, 'wvt_{:d}_{:.0f}_{:.1f}_{:.1f}_{:.1f}_{:.1f}_{:.3f}_{:d}.png')".format(*savename)
...
plt.legend()
#plt.savefig('rate_chg_adapt_%d_%.0f_%.1f_%.1f_%.1f_%.1f_%.3f_%d.png'%savename)
#%%
...
#%%
fig.savefig('fftrate_chg_adapt_%d_%.0f_%.1f_%.1f_%.1f_%.1f_%.3f_%d.png'%savename)
#%%
#plt.figure()
