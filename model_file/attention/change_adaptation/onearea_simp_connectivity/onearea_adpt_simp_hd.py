# -*- coding: utf-8 -*-
"""
Created on Tue May 19 23:34:50 2020
...
loop_num = -1
for chg_adapt_range in np.arange(6,7):
    for ie_ratio in 3.2*np.arange(0.9, 1.06, 0.03):
        for tau_s_de in np.arange(4., 6.1, 0.5):
            for tau_s_di in np.arange(3., 6.1, 0.5): # ms
                for new_delta_gk in np.arange(0, 2.1, 1): # nS
                    for new_tau_k in np.arange(30,51,10):
                        for delta_gk in np.arange(8, 12.1, 1):
                            loop_num += 1
