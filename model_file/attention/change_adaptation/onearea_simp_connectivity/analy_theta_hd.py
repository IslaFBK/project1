...
#%%
savename = (loop_num, spke1.param['chg_adapt_range'],spke1.param['new_delta_gk'], \
 spke1.param['delta_gk'], spke1.param['new_tau_k'],spke1.param['tau_s_di'], \
 spke1.param['tau_s_de'],spke1.param['ie_ratio'])
#%%
...
matcmd = "saveas(gcf, '{:d}_wvt_{:.0f}_{:.0f}_{:.0f}_{:.0f}_{:.1f}_{:.1f}_{:.3f}.png')".format(*savename)
...
plt.xlabel('time(ms)')
plt.ylabel('rate(Hz)')
title = '''adpt_range:%d,new_delta_gk:%.0f,delta_gk:%.0f,new_tau_k:%.0f
...
plt.legend()
plt.savefig('%d_rate_chg_adapt_%.0f_%.0f_%.0f_%.0f_%.1f_%.1f_%.3f.png'%savename)
#%%
...
#%%
fig.savefig('%d_fftrate_chg_adapt_%.0f_%.0f_%.0f_%.0f_%.1f_%.1f_%.3f.png'%savename)
#%%
#plt.figure()
