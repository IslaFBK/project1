# -*- coding: utf-8 -*-
"""
Created on Thu Jun 11 11:58:38 2020

@author: nishe
"""

'''
Low excitatory coupling strength
Simple connectivity: no common neighbour effect
Tune the network to produce spike pattern with jumping behaviour.
And tune the displacement of centre of mass of spike pattern to be stable distribution with heavy tail.
'''
...
sys_argv = int(sys.argv[1])
loop_num = -1
for ie_ratio in 3.375*np.arange(0.7,1.05, 0.03):
    loop_num += 1
    if loop_num == sys_argv:
        print(loop_num)
        break
...
ijwd.ie_ratio = ie_ratio #3.375 #* scale_ie_ratio[i]
...
ijwd.w_ei = 5*10**(-3) #* 1.4 * scale_ei[j]#usiemens
ijwd.w_ii = 25*10**(-3) #* 1.4 * scale_ii[k] #usiemens
...
import levy
#%%
spkrate1, centre_ind1, jump_size1, jump_dist1 = psa.get_rate_centre_jumpdist(spk_e1, starttime=0*ms, endtime=simu_time1, binforrate=10*ms, sample_interval=1*ms, slide_interval = 1, jump_interval = 15, dt=0.1*ms, show_trajectory=False)
...
fit_results_list.insert(0, str(ie_ratio)+',')
...
#ani1 = psa.show_pattern(spkrate1, spkrate2=0, area_num = 1, frames = 5500, bottom_up=1, top_down=1, stimu_onset = -1, start_time = 0, anititle='change adaptation in region near centre at 1000 ms')
#%%
#ani1.save('chg_adapt_1_%s.mp4'%loop_num)
