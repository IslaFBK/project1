# -*- coding: utf-8 -*-
"""
Created on Tue Jun 30 12:51:42 2020
...
# -*- coding: utf-8 -*-
"""
Created on Tue Jun 30 12:00:58 2020
...
# -*- coding: utf-8 -*-
"""
Created on Mon Jun 29 17:27:53 2020
...
#fileadr = '/import/headnode1/shni2598/brian2/data/attention/twoarea_chg_adapt_simp/'
with open('data%s.file'%(loop_num), 'rb') as file:
    spkdata = load_data_dict.data_multiarea(pickle.load(file))
#%%
...
#%%
spkrate2, centre_ind2, jump_size2, jump_dist2 = psa.get_rate_centre_jumpdist(spkdata.a2.e, starttime=0*ms, endtime=10000*ms, binforrate=10*ms, sample_interval=1*ms, slide_interval = 1, jump_interval = 15, dt=0.1*ms, show_trajectory=False)
#%%
'''
plt.figure()
...
#%%
'''
fftcoef = fft(centre_ind2[1000:,1])
magcoef = abs(fftcoef)
...
#%%
savename = (loop_num, spkdata.a2.param['chg_adapt_range'], spkdata.a2.param['new_tau_k'],spkdata.a2.param['new_delta_gk'], \
 spkdata.a2.param['delta_gk'], spkdata.a2.param['tau_s_di'], \
 spkdata.a2.param['tau_s_de'],spkdata.a2.param['ie_ratio'])
#%%
...
matcmd = "saveas(gcf, '{:d}_wvt_{:d}_{:.0f}_{:.1f}_{:.1f}_{:.1f}_{:.1f}_{:.3f}.png')".format(*savename)
...
plt.plot([2000,2000],[0,140],'r--', label='time of adaptation change')
plt.xlabel('time(ms)')
plt.ylabel('rate(Hz)')
...
plt.legend()
plt.savefig('%d_rate_chg_adapt_%d_%.0f_%.1f_%.1f_%.1f_%.1f_%.3f.png'%savename)
#%%
magf = abs(fft(poprate_rate_chgadapt_neu[2000:]-poprate_rate_chgadapt_neu[2000:].mean()))
#%%
...
#%%
fig.savefig('%d_fftrate_chg_adapt_%d_%.0f_%.1f_%.1f_%.1f_%.1f_%.3f.png'%savename)
#%%
#plt.figure()
...
#ani1 = psa.show_pattern(spkrate2, spkrate2=0, area_num = 1, frames = 3000, bottom_up=1, top_down=1, stimu_onset = -1, start_time = 0, anititle=anititle)
#ani1.save('chg_adapt_%d_%.1f_%.1f_%.1f_%.1f_%.3f_%d.png'%savename)
#%%
spkrate1, centre_ind1, jump_size1, jump_dist1 = psa.get_rate_centre_jumpdist(spkdata.a1.e, starttime=0*ms, endtime=10000*ms, binforrate=10*ms, sample_interval=1*ms, slide_interval = 1, jump_interval = 15, dt=0.1*ms, show_trajectory=False)
spkrate1 = psa.overlap_centreandspike(centre_ind1, spkrate1, show_trajectory = False)
anititle=''#'change adaptation in region near centre to %.1fnS at 1000 ms\ndelta_gk: %.1fnS; tau_s_di: %.1fms; ie_ratio: %.3f'%(spke1.param['new_delta_gk'], spke1.param['delta_gk'], spke1.param['tau_s_di'], spke1.param['ie_ratio'])
ani1 = psa.show_pattern(spkrate1, spkrate2, area_num = 2, frames = 2000, bottom_up=1, top_down=1, stimu_onset = -1, start_time = 0, anititle=anititle)
'''
