# -*- coding: utf-8 -*-
"""
Created on Tue Jun 30 10:59:54 2020
...
#%%
spk_3 = SpikeMonitor(group_e_2, record = True)
#%%
net = Network(collect())
net.store('state1')
#%%
...
spk_3_tstep = np.round(spk_3.t/(0.1*ms)).astype(int)

#param_1 = {'delta_gk':10,'tau_s_di':3,'tau_s_de':5,'ie_ratio':3.1725,'sti_onset':'10s'}

param_3 = {'chg_adapt_time':'2s','delta_gk':12,'new_delta_gk':2,'new_tau_k':40,'tau_s_di':3.5,\
...
data = {'a2':{'param':param_3,'e':{'i':spk_3.i[:],'t':spk_3_tstep}}}
...
#%%
spkrate3, centre_ind3, jump_size3, jump_dist3 = psa.get_rate_centre_jumpdist(spkdata2.a2.e, starttime=0*ms, endtime=500*ms, binforrate=10*ms, sample_interval=1*ms, slide_interval = 1, jump_interval = 15, dt=0.1*ms, show_trajectory=False)
