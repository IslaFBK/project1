...
# -*- coding: utf-8 -*-
"""
Created on Mon Apr 12 15:37:18 2021
...
start_time_ls = [0, 200, 500]
anly_dura_ls = [200, 500, 1000]
    ...
    for start_time in start_time_ls:
        for anly_dura  in anly_dura_ls:
            ...
            for st in range(n_StimAmp):    
                ...
                ax[0].plot(m_v_1[st,:,0,:].reshape(-1), m_v_1[st,:,1,:].reshape(-1), '.', c=clr[0], label='sens; no-att')
                ax[0].plot(m_v_att_1[st,:,0,:].reshape(-1), m_v_att_1[st,:,1,:].reshape(-1), '.', c=clr[1], label='sens; att')
                               
                max_lim = max(*ax[0].get_xlim(), *ax[0].get_ylim())
                min_lim = min(*ax[0].get_xlim(), *ax[0].get_ylim())
                
                ax[0].set_xlim([min_lim,max_lim])
                ax[0].set_ylim([min_lim,max_lim])
                
                ax[0].plot([min_lim, max_lim],[min_lim, max_lim], ls='--')
                
                ax[1].plot(m_v_2[st,:,0,:].reshape(-1), m_v_2[st,:,1,:].reshape(-1), '.', c=clr[0], label='asso; no-att')
                ax[1].plot(m_v_att_2[st,:,0,:].reshape(-1), m_v_att_2[st,:,1,:].reshape(-1), '.', c=clr[1], label='asso; att')
                               
                max_lim = max(*ax[1].get_xlim(), *ax[1].get_ylim())
                min_lim = min(*ax[1].get_xlim(), *ax[1].get_ylim())
                
                ax[1].set_xlim([min_lim,max_lim])
                ax[1].set_ylim([min_lim,max_lim])
                
                ax[1].plot([min_lim, max_lim],[min_lim, max_lim], ls='--')                
