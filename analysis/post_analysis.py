...
#    t_spk = (np.round(spike.t/dt)).astype(int)
    spiket = np.round(spike.t/dt).astype(int)
...
#        spk_rate[:, t][neu] += counts
    if indiv_rate:
        ...
        pop_rate = (spk_rate.sum(0).sum(0))/(window/(1*second))/n_neuron
    else:
        pop_rate = spk_rate.sum(0)/(window/(1*second))/n_neuron
...
def get_rate_centre_jumpdist(spk, starttime, endtime, binforrate=10*ms, sample_interval=1*ms, slide_interval = 1, jump_interval = 15, dt=0.1*ms):
    '''
    get firing rate, centre of mass of pattern and pattern jump distance
    
    #starttime: start calulation time
    #endtime: end calculation time
    #binforrate: duration of bin for calculating the firing rate at each timestep 
    #sample_interval: interval between two ajacent timesteps for firing rate calculation
    #slide_interval: the time steps interval between two successive centre-of-mass calculation.
    #jump_interval: the time steps interval between two successive centre-of-mass jump distance calculation
    '''
    spkrate = get_spike_rate(spk, start_time=starttime, end_time=endtime, sample_interval = sample_interval, \
                                indiv_rate = True, popu_rate = False,\
                                n_neuron = 3969, window = binforrate, dt = dt)
    
    centre_ind, jump_size, jump_dist = get_centre_mass(spkrate, slide_interval=slide_interval, jump_interval=jump_interval)
    
    #spkrate = overlap_centreandspike(centre_ind, spkrate[:,:,:], show_trajectory = show_trajectory)
    
    return spkrate, centre_ind, jump_size, jump_dist
...
def plot_traj(ax, centre_ind, corner = np.array([[-0.5,-0.5],[62.5, 62.5]]), coordinate_type_is_matrix=True):
    ...
    return ax
#%%
def show_pattern(spkrate1, spkrate2=0, area_num = 1, frames = 1000, start_time = 0, anititle='animation'):
    if area_num == 1:
        ...
        ani=animation.FuncAnimation(fig, updatev, frames=frames, interval=20)    # frames=spk1.shape[2]
        return ani
    else:
